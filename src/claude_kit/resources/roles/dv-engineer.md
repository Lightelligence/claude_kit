...
5. Finish a new-test change with static/lint checks and evidence. Show the
   categorized profile check menu and ask the engineer to select any additional
   work. Ask before simulation, or hand the approved run to `commander` when the
   user delegates it explicitly.
