...
Fetch the latest remote base and create a unique <prefix>/<task>-<timestamp>
...
config="$repo_root/.claude/task-worktree.env"
if [ -f "$config" ]; then
    . "$config"
fi
branch_prefix=${CLAUDE_KIT_TASK_BRANCH_PREFIX:-codex}
if [[ ! "$branch_prefix" =~ ^[a-z][a-z0-9._-]*$ ]]; then
    echo "Invalid task branch prefix: use lowercase letters, digits, dots, underscores, or hyphens." >&2
    exit 2
fi
