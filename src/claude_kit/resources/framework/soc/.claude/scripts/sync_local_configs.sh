...
the current checkout). If CLAUDE_KIT_LOCAL_CONFIG_ROOT or the repository-local
Git key claudeKit.localConfigRoot is set, configuration is linked from that
persistent directory instead. The older VIBE_SOC names are still supported.
EOF
}
...
configured_root=${CLAUDE_KIT_LOCAL_CONFIG_ROOT:-${VIBE_SOC_LOCAL_CONFIG_ROOT:-}}
if [ -z "$configured_root" ]; then
    configured_root=$(git -C "$source_worktree" config --local --get claudeKit.localConfigRoot 2>/dev/null || true)
fi
if [ -z "$configured_root" ]; then
