...
ROOT = project_root()
TARGET = ROOT / ".claude/scripts/sync_mcp_runtime.py"
...
def main() -> int:
    ...
    if OLD not in text:
        ...
        return 2
    if not args.write:
        print("[MCP-LOCK] OUT-OF-DATE: .claude/scripts/sync_mcp_runtime.py", file=sys.stderr)
        return 2
    TARGET.write_text(text.replace(OLD, NEW, 1), encoding="utf-8")
    print("[MCP-LOCK] Wrote .claude/scripts/sync_mcp_runtime.py")
    return 0
