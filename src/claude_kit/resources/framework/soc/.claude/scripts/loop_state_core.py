...
STAGE_REQUIRED_CHECK_GROUPS = {
    ...
    "syn": (("registered synthesis", frozenset({"soc_syn"})), ),
}
...
def dependencies_satisfied(pipeline: dict, stage: str) -> bool:
    return all(pipeline.get(dep, {}).get("status") in SUCCESS_STATES for dep in DEPENDENCIES.get(stage, ()))
...
def recompute_blocked(pipeline: dict) -> None:
    for stage in STAGE_ORDER:
        ...
        if not isinstance(info, dict):
            continue
        unsatisfied = [dep for dep in DEPENDENCIES[stage] if pipeline.get(dep, {}).get("status") not in SUCCESS_STATES]
...
def compute_next_actions(pipeline: dict) -> list[dict]:
    ...
    if failures:
        ...
        for stage in failures:
            unsatisfied = [
                dep for dep in DEPENDENCIES[stage] if pipeline.get(dep, {}).get("status") not in SUCCESS_STATES
            ]
            if unsatisfied:
                actions.append({
                    "stage": unsatisfied[0],
...
def validate_artifacts(workspace: Path, artifacts: list[str], *, require_existing: bool) -> list[str]:
    validated = []
...
def build_artifact_evidence(workspace: Path, artifacts: list[str], checks: list[dict]) -> dict:
    evidence = {}
    provenance = sorted({
        str(check.get("tool"))
        for check in checks if isinstance(check, dict) and check.get("passed") and check.get("tool")
    })
    for item in artifacts:
        rel, path = artifact_path(workspace, item)
        if not path.exists():
...
def _manifest_material(manifest: Path, repo: Path, seen: set[Path]) -> set[Path]:
...
def _compute_rtl_fingerprint(workspace: Path, *, include_generated_manifest: bool) -> str | None:
    ...
    if rtl_dir.is_dir():
        material.update(path.resolve() for path in rtl_dir.rglob("*")
...
def missing_required_checks(stage: str, checks: list[dict], *, legacy_contract: bool = False) -> list[str]:
    tools = [
        str(check.get("tool")) for check in checks
        if isinstance(check, dict) and check.get("passed") and check.get("tool")
    ]
    return [
        label for label, aliases in (
            LEGACY_STAGE_REQUIRED_CHECK_GROUPS if legacy_contract else STAGE_REQUIRED_CHECK_GROUPS)[stage]
        if not any(_tool_matches(tool, family) for tool in tools for family in aliases)
    ]


def stage_artifact_issues(stage: str, artifacts: list[str]) -> list[str]:
    if stage == "doc":
        return [] if any(rel.startswith("docs/") and Path(rel).suffix.lower() == ".md"
                         for rel in artifacts) else ["doc requires at least one docs/*.md artifact"]
    if stage == "rtl":
        has_filelist = any(
            rel.startswith("de/rtl/") and Path(rel).name in {"filelist.f", "filelist.mk"} for rel in artifacts)
        return [] if has_filelist else ["rtl requires de/rtl/filelist.f or de/rtl/filelist.mk"]
    if stage == "verif":
        return [] if any(rel.startswith("dv/sim/") and rel.endswith(".log")
                         for rel in artifacts) else ["verif requires a dv/sim/*.log artifact"]
    if stage == "syn":
        has_output = any(
            rel.startswith("de/syn/") and (
                rel.endswith(("_netlist.v", "_netlist.sv")) or Path(rel).name in {"synth.log", "yosys.log"})
            for rel in artifacts)
        return [] if has_output else ["syn requires a synthesis log or netlist artifact"]
    return [f"unknown stage: {stage}"]


def stage_claim_issues(stage: str, artifacts: list[str], checks: list[dict]) -> list[str]:
    """Cross-check optional claims whose evidence is not mandatory for a stage."""
    if stage != "syn":
        return []
    has_timing = any(
        rel.startswith("de/syn/") and rel.endswith(".rpt") and "timing" in Path(rel).name.lower() for rel in artifacts)
    timing_claimed = any(
        isinstance(result, dict) and result.get("passed") and any(
            _tool_matches(str(result.get("tool") or ""), family) for family in ("timing", "check_timing"))
...
def _valid_evidence_record(record: object) -> bool:
    ...
    return bool(
        isinstance(sha256, str) and re.fullmatch(r"[0-9a-f]{64}", sha256) and isinstance(size, int)
        and not isinstance(size, bool) and size >= 0 and isinstance(recorded_at, str) and recorded_at
        and isinstance(provenance, list) and provenance and all(isinstance(item, str) and item for item in provenance))
...
def _validate_stage(
    ...
    *,
    ...
    for check_result in checks:
        malformed = (not isinstance(check_result, dict) or not check_result.get("tool")
    ...
    if not isinstance(evidence, dict):
        _issue(issues, "error", "artifact_evidence must be an object", module=module, stage=stage)
        evidence = {}
    ...
    if status == "done":
        ...
        if not valid_checks:
            _issue(issues, "error", "done stage has no check_results", module=module, stage=stage)
        ...
        if stage in {"verif", "syn"}:
            ...
            valid_run = (isinstance(run_evidence, dict) and isinstance(run_evidence.get("run_id"), str)
                         and bool(run_evidence.get("run_id"))
                         and (str(run_evidence.get("run_id")).startswith(expected_family + "-")
                         ...
                         and _tool_matches(run_evidence.get("tool_family"), expected_family))
            if not valid_run:
                _issue(
                    issues,
                    _legacy_severity(legacy, info),
            ...
            elif (verify_filesystem and current_fingerprint and fingerprint != current_fingerprint):
                _issue(
                    issues,
                    "warning" if allow_material_drift else "error",
...
def _validate_pipeline(
    ...
    *,
    ...
    for missing in sorted(expected - actual):
        _issue(issues, "error", f"missing pipeline stage {missing!r}", module=module, stage=missing)

    current_fingerprint = compute_rtl_fingerprint(workspace) if verify_filesystem else None
    downstream_repair = any(
        isinstance(pipeline.get(name), dict) and pipeline[name].get("status") == "in_progress"
    ...
    for stage in STAGE_ORDER:
        if stage in pipeline:
            _validate_stage(
                ...
                allow_material_drift=(downstream_repair and stage in {"rtl", "verif", "syn"}
    ...
    fingerprints = {
        ...
        for stage in ("rtl", "verif", "syn") if isinstance(pipeline.get(stage), dict)
        and pipeline[stage].get("status") == "done" and pipeline[stage].get("rtl_fingerprint")
    }
    ...
    for stage, deps in DEPENDENCIES.items():
        ...
        unsatisfied = [
            dep for dep in deps
            if not isinstance(pipeline.get(dep), dict) or pipeline[dep].get("status") not in SUCCESS_STATES
        ]
        status = info.get("status")
...
def validate_state(
    ...
    *,
    ...
    if legacy:
        severity = ("warning" if allow_legacy and version in LEGACY_SCHEMA_VERSIONS else "error")
        _issue(
            issues,
            severity,
...
def _format_issues(issues: list[dict]) -> str:
    lines = []
    for issue in issues:
        where = "/".join(part for part in (issue.get("module"), issue.get("stage")) if part)
...
def _state_containers(state: dict) -> list[tuple[str, dict]]:
    if state.get("mode") == "multi_module":
        return [(module, module_state) for module, module_state in (state.get("modules") or {}).items()
...
def _legacy_rtl_fingerprint(pipeline: dict) -> str | None:
    ...
    text.extend(str(result.get("note") or "") for result in rtl.get("check_results") or [] if isinstance(result, dict))
    for value in text:
...
def normalize_legacy_state(state: dict, workspace: Path) -> tuple[dict, list[str]]:
    ...
    if original_version not in LEGACY_SCHEMA_VERSIONS:
        raise ValueError(f"unsupported schema_version {original_version!r}; refusing implicit downgrade")
    ...
    normalized.setdefault("migration", {}).update({
        ...
        "normalization":
        "current-checkout snapshot; legacy checks not re-executed",
    })
    changes.extend([
        ...
        for stage, info in pipeline.items():
            ...
            if info.get("status") == "done":
                ...
                if stage in {"rtl", "verif", "syn"} and current_fingerprint:
                    info["rtl_fingerprint"] = (legacy_rtl_fingerprint
                                               if stage == "rtl" and legacy_rtl_fingerprint else current_fingerprint)
                    info["rtl_fingerprint_source"] = "migration_current_snapshot"
            elif (stage in {"verif", "syn"} and info.get("status") == "in_progress" and current_fingerprint):
                info["evidence_policy"] = "legacy_compatible"
...
def _validate_transition(pipeline: dict, stage: str, target: str, note: str) -> None:
    ...
    if target == "in_progress":
        other_failures = [
            name for name in STAGE_ORDER if name != stage and pipeline.get(name, {}).get("status") == "fail"
        ]
        if other_failures:
...
def _run_binding(
    ...
    if not source_fingerprint and not legacy:
        raise ValueError(f"{stage} closure requires --source-fingerprint from the MCP run")
    claimed = source_fingerprint or current
    normalized_legacy_fingerprint = False
    if claimed != current:
        legacy_current = _compute_rtl_fingerprint(workspace, include_generated_manifest=True)
        if claimed != legacy_current:
            raise ValueError(f"{stage} MCP run consumed source fingerprint {claimed}, "
    ...
    if run_id and not legacy and not run_id.startswith(tool_family + "-"):
        raise ValueError(f"{stage} run ID must be emitted by {tool_family} (expected {tool_family}-...)")
    matching = [
        str(result.get("tool")) for result in checks
        if isinstance(result, dict) and result.get("passed") and _tool_matches(str(result.get("tool")), tool_family)
    ]
    evidence = {
...
def _settle_downstream_repair(
    ...
    if owner and owner != stage:
        raise ValueError(f"RTL repair is already owned by {owner}; reopen rtl before {stage} edits RTL")
    if not run_evidence:
        raise ValueError(f"{stage} changed RTL and must provide MCP run source fingerprint/run ID")
    repair_checks = checks or []
    missing = missing_required_checks("rtl", repair_checks)
    if missing:
        raise ValueError(f"{stage} changed RTL; repair closure lacks RTL checks: {', '.join(missing)}")
    checker_result = _call_checker("rtl", workspace, "", rtl, policy="closure")
    if not checker_result.get("passed"):
        raise ValueError("downstream RTL repair checker failed: " +
                         "; ".join(checker_result.get("issues") or ["unknown RTL checker failure"]))
    rtl_checks = [
        result for result in repair_checks if isinstance(result, dict) and any(
            _tool_matches(str(result.get("tool") or ""), family) for family in (
                "soc_lint",
                "soc_comp",
                "rtl_quality",
    ...
    rtl["artifact_evidence"] = build_artifact_evidence(workspace, rtl.get("artifacts") or [], rtl_checks)
...
def _apply_transition(
    ...
    if status == "in_progress":
        ...
        validated_artifacts = validate_artifacts(workspace, artifacts, require_existing=True)
        ...
        candidate = {
            ...
            "artifact_evidence": build_artifact_evidence(workspace, validated_artifacts, parsed_checks),
        }
        checker_result = _call_checker(stage, workspace, checker_module, candidate, policy="closure")
        if not checker_result.get("passed"):
            raise ValueError(f"{stage} real evidence checker failed: " +
        ...
        info.update({
            ...
            "rtl_fingerprint_source": ("recorded_at_transition" if fingerprint else None),
        ...
        validated_artifacts = (validate_artifacts(workspace, artifacts, require_existing=True) if artifacts else [])
        fingerprint = info.get("rtl_fingerprint")
        run_evidence = None
        legacy_binding = False
        if stage in {"verif", "syn"}:
        ...
        info.update({
            ...
            validated_artifacts,
            "artifact_evidence":
            build_artifact_evidence(workspace, validated_artifacts, parsed_checks),
            "check_results":
            parsed_checks,
            "notes":
            note,
            "rtl_fingerprint":
            fingerprint,
            "rtl_fingerprint_source": ("failed_mcp_run" if run_evidence else info.get("rtl_fingerprint_source")),
        })
        if run_evidence:
            info["run_evidence"] = run_evidence
        if legacy_binding:
        ...
        if stage in {"verif", "syn"}:
            _settle_downstream_repair(pipeline, workspace, stage, status, None)
    ...
    *,
    ...
    with lock_path.open("a+") as lock:
        ...
        if state_errors(pre_issues):
            raise ValueError("state is invalid before update; run migrate_state.py first: " +
        ...
        if state_errors(post_issues):
            raise ValueError("transition produced invalid state: " + _format_issues(state_errors(post_issues)))
        _atomic_write(state_path, state)
...
def _print_stats(pipeline: dict) -> tuple[int, int, int]:
    ...
    done = sum(1 for stage in STAGE_ORDER if pipeline.get(stage, {}).get("status") in SUCCESS_STATES)
    failed = sum(1 for stage in STAGE_ORDER if pipeline.get(stage, {}).get("status") == "fail")
...
def _compact_pipeline(pipeline: dict, current_fingerprint: str | None) -> dict:
    ...
    for stage in STAGE_ORDER:
        ...
        if stage in {"rtl", "verif", "syn"} and info.get("status") == "done":
            fingerprint_current = bool(current_fingerprint and recorded_fingerprint == current_fingerprint)
        stages[stage] = {
            "status": info.get("status", "missing"),
...
def _compact_issue_actions(issues: list[dict]) -> list[dict]:
    ...
    for issue in issues:
        ...
        action = ("repair_evidence" if stage in STAGE_ORDER and any(term in message.lower()
                                                                    for term in evidence_terms) else "repair_state")
        reason = message[:240]
    ...
    *,
    ...
    summary = {
        ...
        "issues": {
            ...
            "codes": sorted({str(issue.get("code"))
                             for issue in issues if issue.get("code")}),
        },
    ...
    if state.get("mode") == "multi_module":
        ...
        summary["modules"] = {
            module: {
                "stages":
                _compact_pipeline(module_state.get("pipeline", {}), current_fingerprint),
                "next_actions":
                _merge_compact_actions(
                    _compact_issue_actions(
                        [issue for issue in errors if not issue.get("module") or issue.get("module") == module]),
        ...
        summary["stages"] = _compact_pipeline(state.get("pipeline", {}), current_fingerprint)
        summary["next_actions"] = _merge_compact_actions(_compact_issue_actions(errors),
...
def _has_passed_tool(info: dict, *families: str) -> bool:
    return any(
        isinstance(result, dict) and result.get("passed") and any(
            _tool_matches(str(result.get("tool") or ""), family) for family in families)
...
def _check_recorded_docs(workspace: Path, info: dict) -> dict:
    details = {}
    issues = []
    docs = [rel for rel in info.get("artifacts") or [] if rel.startswith("docs/") and Path(rel).suffix.lower() == ".md"]
    if not docs:
        return {
            "passed": False,
            "details": details,
...
def _call_checker(
    ...
    *,
    ...
    if stage == "verif":
        ...
        logs = [rel for rel in info.get("artifacts") or [] if rel.startswith("dv/sim/") and rel.endswith(".log")]
        existing = [rel for rel in logs if (workspace / rel).is_file()]
        if not existing:
            retained = info.get("artifact_evidence") or {}
            saved = any(
                isinstance(retained.get(rel), dict) and retained[rel].get("sha256") and retained[rel].get("provenance")
                for rel in logs)
            return {
                "passed": False,
    ...
    if stage == "syn":
        ...
        reports = [
            rel for rel in info.get("artifacts") or []
            if rel.startswith("de/syn/") and rel.endswith(".rpt") and "timing" in Path(rel).name.lower()
        ]
        ...
        if missing:
            retained = info.get("artifact_evidence") or {}
            saved = all(_valid_evidence_record(retained.get(rel)) for rel in missing)
            return {
                "passed": False,
                "unavailable_with_retained_evidence": saved,
                "issues": ["recorded STA timing report is unavailable in this checkout: " + ", ".join(missing)],
                "details": {
                    "reports": reports,
                    "missing": missing
                },
        ...
        issues = [
            f"{report}: {message}" for report, result in zip(existing, results)
...
def _run_done_stage_checks(state: dict, workspace: Path, issues: list[dict], details: dict) -> None:
    ...
    for module, container in _state_containers(state):
        ...
        for stage in STAGE_ORDER:
            ...
            if result.get("passed"):
                continue
            severity = ("warning" if result.get("unavailable_with_retained_evidence") else "error")
            for message in result.get("issues") or ["evidence checker failed"]:
                _issue(
                    ...
                    module=module,
                    stage=stage,
                    code=("retained_evidence_needs_revalidation"
                          if severity == "warning" else "evidence_checker_failed"),
                )
...
def _is_material_path(path: str, workspace_rel: str) -> bool:
    if workspace_rel != "." and not (path == workspace_rel or path.startswith(workspace_rel + "/")):
        return False
    normalized = "/" + path
    return any(part in normalized for part in MATERIAL_PATH_PARTS) or (Path(path).name in MATERIAL_FILENAMES)
...
def _migration_stage_reasons(
    ...
    *,
    ...
    if stage in {"rtl", "verif", "syn"}:
        current_fingerprint = compute_rtl_fingerprint(workspace)
        recorded_fingerprint = info.get("rtl_fingerprint")
        if (current_fingerprint and recorded_fingerprint and recorded_fingerprint != current_fingerprint):
    ...
    elif any(not isinstance(result, dict) or not result.get("passed") for result in checks):
        reasons.append("recorded checks are malformed or failed")
    missing = missing_required_checks(stage, checks, legacy_contract=legacy_contract)
    if missing:
        reasons.append(f"required checks absent: {', '.join(missing)}")
    reasons.extend(stage_claim_issues(stage, artifacts, checks))
    if not reasons:
        checker_module = module if multi and stage == "doc" else ""
        result = _call_checker(stage, workspace, checker_module, info, policy="migration")
        if not result.get("passed"):
            reasons.extend(result.get("issues") or ["real evidence checker failed"])
    return reasons


def migrate_state_data(state: dict, workspace: Path) -> tuple[dict, list[str], list[dict]]:
    ...
    if original_version == CURRENT_SCHEMA_VERSION:
        ...
        if (isinstance(migration, dict) and migration.get("migrated_from_schema") == CURRENT_SCHEMA_VERSION):
    ...
    for module, container in _state_containers(migrated):
        ...
        for stage in STAGE_ORDER:
            ...
            if info.get("status") == "done":
                reasons = ([f"dependencies are not complete: {unsatisfied}"]
                           if unsatisfied else _migration_stage_reasons(
                               workspace,
                               module,
                               stage,
                               info,
                ...
                if reasons:
                    ...
                    if stage in {"rtl", "verif", "syn"}:
                        ...
                        if stage in {"verif", "syn"} and fingerprint:
                            ...
                            tool = next(
                                (str(result.get("tool"))
                                 for result in info.get("check_results") or [] if isinstance(result, dict)
                                 and result.get("passed") and _tool_matches(str(result.get("tool") or ""), family)),
                                family,
                            )
                            info["run_evidence"] = {
                ...
                _reset_for_migration(info, f"dependencies are not complete: {unsatisfied}")
    ...
    if original_version != CURRENT_SCHEMA_VERSION:
        ...
        migrated.setdefault("migration", {}).update({
            "migrated_from_schema": (original_version if original_version is not None else 1),
            "validated_at":
            now(),
            "policy":
            "done retained only with current real evidence",
        })
    elif changes:
        migrated["last_updated"] = now()
        migrated.setdefault("repair", {}).update({
            ...
            now(),
            "policy":
            "invalid current evidence downgraded without rewriting valid provenance",
        })
    if multi:
...
def _active_state_stages(state: dict) -> list[str]:
    ...
    for module, container in _state_containers(state):
        ...
        active.extend(f"{module}/{stage}" for stage, info in pipeline.items()
...
def _migration_result(
    ...
    *,
    ...
    if active:
        if write:
            raise ValueError("refusing to migrate state with active stages: " + ", ".join(active))
        return {
    ...
    errors = state_errors(issues)
    if errors:
        raise ValueError("migration could not produce valid state: " + _format_issues(errors))
    return {
        "workspace": str(workspace_path),
        "state_path": str(state_path),
        "dry_run": not write,
...
def migrate_state(workspace: str, *, write: bool = False) -> dict:
    ...
    if write:
        ...
        with lock_path.open("a+") as lock:
            fcntl.flock(lock.fileno(), fcntl.LOCK_EX)
            state = json.loads(state_path.read_text(encoding="utf-8"))
            result = _migration_result(workspace_path, state_path, state, write=True)
...
def migrate_main() -> int:
    parser = argparse.ArgumentParser(description="Migrate pipeline_state.json to the canonical schema")
    parser.add_argument("workspace")
    parser.add_argument(
        "--write",
        action="store_true",
