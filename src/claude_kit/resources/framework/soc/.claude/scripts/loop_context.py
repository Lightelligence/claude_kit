...
def _changed_lines(repo: Path, base_ref: str, path: str) -> str:
    ...
    return "\n".join(line[1:] for line in output.splitlines()
...
def _requested_mode(value: str | None, policy: dict) -> str:
    requested = (os.environ.get("LOOP_MODE") if value in {None, "auto"} else value) or policy.get("default_mode", "dev")
    if requested == "auto":
    ...
    for mode in MODE_RANK:
        ...
        for field in (
            ...
            if value > EXECUTION_MAXIMA[mode][field]:
                raise ValueError(f"loop policy {mode} mode {field} exceeds its safe maximum")
        if (execution["instruction_budget_words"] == 0 or execution["max_parallel_owners"] == 0):
...
def route_mode(
    ...
    pipeline_paths = [path for path in material_paths if not _matches(path, routing["lightweight_globs"])]
    affected = sorted({item for path in pipeline_paths if (item := module_workspace(path))})
    material = bool(pipeline_paths)

    signoff_paths = [path for path in pipeline_paths if _matches(path, routing["signoff_globs"])]
    merge_paths = [path for path in pipeline_paths if _matches(path, routing["merge_globs"])]
    ...
    if material and (explicit_signoff or semantic_signoff):
        detected = "signoff"
        reasons.append("signoff-sensitive impact: " + ", ".join(explicit_signoff + semantic_signoff))
    if detected != "signoff" and merge_paths:
        detected = "merge"
        reasons.append(f"delivery-sensitive paths: {', '.join(merge_paths[:4])}")
    if not reasons:
        reasons.append("single-module low-risk inner-loop change" if material else (
            "lightweight verification manifest change" if material_paths else "no pipeline-governed risk detected"))
...
def _stage_freshness(
    ...
    if not state or state.get("mode") == "multi_module":
        return {stage: {"fresh": False, "reason": "no single-module state"} for stage in STAGE_ORDER}
    ...
    stage_changes = {
        ...
        "rtl": [path for path in relative_paths if path.startswith("de/rtl/") or path == "de/run/rtl.f"],
    ...
    for stage in STAGE_ORDER:
        ...
        recorded_paths = set(info.get("artifacts") or []) | set((info.get("artifact_evidence") or {}).keys())
        unrecorded_changes = [path for path in stage_changes[stage] if path not in recorded_paths]
        stage_errors = [issue for issue in issues if issue.get("severity") == "error" and issue.get("stage") == stage]
        fingerprint_ok = True
        ...
        if not status_ok:
            ...
            reason = "changed paths are not bound to stage evidence: " + ", ".join(unrecorded_changes[:3])
        elif stage_errors:
...
def build_context(
    ...
    *,
    ...
    repo = find_repo_root(workspace)
    policy_path = policy_path or project_root() / ".claude/loop_policy.json"
    policy = load_policy(policy_path)
    base_ref = base_ref or _default_base(repo)
    all_paths = sorted({
        _normalize_path(path, repo)
        for path in (changed_paths if changed_paths is not None else collect_changed_paths(repo, base_ref))
    })
    ...
    if governed and mode in {"merge", "signoff"}:
        stage_evidence_ready = bool(state and not state_errors(issues)
    ...
    delivery_ready = bool(stage_evidence_ready and (not risk_checks_required or risk_checks_passed)
    ...
    for stage, rule in (
        ("rtl", ".claude/rules/10_rtl_change_gate.md"),
        ("verif", ".claude/rules/11_verif_recovery_gate.md"),
        ("syn", ".claude/rules/12_syn_pd_gate.md"),
    ):
    ...
    if rtl_work:
        ...
        full_style = ".claude/references/04_verilog_coding_style.md"
    ...
    if not governed:
        ...
        required_checks = _dev_required_checks(affected_stage_list, mode_policy["required_checks"])
    else:
...
def _print_text(context: dict) -> None:
    ...
    print(f"Diff scope    : {context['scope']} "
    ...
    cache_text = ", ".join(f"{stage}={'fresh' if info['fresh'] else 'stale'}"
...
def main() -> int:
    ...
    if args.write:
        if context["workspace"] == "." or not context["pipeline_governed"]:
            ...
            return 1
        output = (args.workspace.expanduser().resolve() / "de" / "run" / "loop_evidence" / "loop_context.json")
