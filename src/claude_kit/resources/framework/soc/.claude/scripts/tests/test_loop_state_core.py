...
class WorkspaceCase(unittest.TestCase):
    ...
    def load(self) -> dict:
        return json.loads((self.workspace / "pipeline_state.json").read_text(encoding="utf-8"))
    ...
    def test_downstream_evidence_is_bound_to_rtl_snapshot(self) -> None:
        ...
        self.assertTrue(
            any(issue.get("code") == "rtl_fingerprint_mismatch" or "stale" in issue["message"] for issue in issues))
    ...
    def test_compact_summary_turns_invalid_evidence_into_next_action(self) -> None:
        ...
        repair = [
            action for action in summary["next_actions"]
    ...
    def test_legacy_generated_manifest_fingerprint_is_normalized(self) -> None:
        ...
        reported = core._compute_rtl_fingerprint(self.workspace, include_generated_manifest=True)
        ...
        self.assertEqual(evidence["fingerprint_normalization"], "generated_manifest_v1")
    ...
    def test_transient_digest_becomes_warning_on_clean_clone(self) -> None:
        ...
        log.unlink()
        issues = core.validate_state(state, self.workspace)
        matching = [issue for issue in issues if issue.get("code") == "transient_evidence_only"]
        self.assertEqual(len(matching), 1)
        self.assertEqual(matching[0]["severity"], "warning")
        self.assertFalse(any(issue["severity"] == "error" and "dv/sim/sim.log" in issue["message"] for issue in issues))
    ...
    def test_legacy_normalization_marks_current_snapshot(self) -> None:
        ...
        self.assertEqual(normalized["pipeline"]["rtl"]["evidence_policy"], "legacy_compatible")
    ...
    def test_legacy_tool_suffix_and_doc_review_survive_migration(self) -> None:
        ...
        state["pipeline"]["rtl"]["check_results"] = [
            result for result in state["pipeline"]["rtl"]["check_results"] if result["tool"] != "soc_lint"
        ]
        write(self.workspace / "dv" / "sim" / "sim.log", "RESULT: ALL TESTS PASS\n")
