...
class LoopContextCase(unittest.TestCase):
    ...
    def test_low_risk_single_module_rtl_uses_dev(self) -> None:
        ...
        self.assertIn(".claude/rules/04_coding_style.md", result["rules"])
    ...
    def test_doc_only_dev_skips_rtl_coding_style_injection(self) -> None:
        ...
        self.assertNotIn(
            ".claude/references/04_verilog_coding_style.md",
    ...
    def test_constraint_and_explicit_interface_impact_escalate_to_signoff(self) -> None:
        ...
        explicit = self.context(["ip/digital/demo/de/rtl/demo.sv"], impacts={"interface"})
    ...
    def test_write_uses_module_de_run_and_refuses_repo_root(self) -> None:
        ...
        self.assertTrue((self.workspace / "de/run/loop_evidence/loop_context.json").is_file())
    ...
    def test_final_evidence_reuses_current_fingerprint(self) -> None:
        self.close_pipeline()
        before_review = self.context(["ip/digital/demo/de/rtl/demo.sv"], mode="merge")
        result = self.context(
            ["ip/digital/demo/de/rtl/demo.sv"],
            mode="merge",
            review_result="pass",
        ...
        self.assertFalse(result["execution"]["preflight"]["required"])
        self.assertLess(len(json.dumps(result)), 4096)
        self.assertTrue(all(item["fresh"] for item in result["cache"]["stages"].values()))
