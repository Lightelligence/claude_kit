...
class SocBuildMcpTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self.env_patcher = patch.dict(os.environ, {SERVER.MCP_SERVER_ACTIVE_ENV: "1"})
        ...
        (run_dir / "rtl.f").write_text(str(rtl / "demo.sv") + "\n", encoding="utf-8")
    ...
    def write_syn_snapshot(self, *, upf: bool) -> tuple[str, str, Path]:
        ...
        run_id = "soc_syn-" + "a" * 32
        native = (self.module_dir / "de" / "syn" / "loop_evidence" / run_id / "native" / "de" / "syn")
        native.mkdir(parents=True)
        (native / "rtl.f").write_text(
            str(self.module_dir / "de" / "rtl" / "demo.sv") + "\n",
            encoding="utf-8",
        )
        (native / "demo_netlist.v").write_text("module demo; endmodule\n", encoding="utf-8")
        (native / "demo.svf").write_text("svf\n", encoding="utf-8")
        if upf:
            (native / "upf").mkdir()
            (native / "upf" / "demo.upf").write_text("set_design_top demo\n", encoding="utf-8")
            (native / "upf" / "demo_synth.upf").write_text("set_design_top demo\n", encoding="utf-8")
        structural = {
    ...
    def write_syn_contract(self, *, top: str, syn_tool: str, upf: bool = False) -> dict[str, Path]:
        ...
        if upf:
            ...
            contract["reference_upf"].write_text(f"set_design_top {top}\n", encoding="utf-8")
        values = {
    ...
    def write_syn_products(self, contract: dict[str, Path]) -> None:
        ...
        if "implementation_upf" in contract:
            contract["implementation_upf"].write_text("set_design_top demo\n", encoding="utf-8")
    ...
    def write_formal_reports(self, *, upf: bool) -> None:
        ...
        reports = {
            ...
            "setup_status.rpt":
            "setup clean\n",
            "verification_status.rpt": ("Status:             SUCCEEDED\n"
                                        "Failing Points:     0\n"
                                        "Aborted Points:     0\n"
                                        "Unverified Points:  0\n"
    ...
    def test_tool_registry_contains_new_interfaces(self) -> None:
        ...
        self.assertTrue(
            {"soc_sim", "soc_regress", "soc_coverage", "soc_syn", "soc_formal", "soc_verdi", "soc_cdc"} <= set(tools))

    def test_rejects_direct_tool_object_invocation(self) -> None:
        with patch.dict(os.environ, {}, clear=True):
            with self.assertRaisesRegex(RuntimeError, "registered soc-build MCP server"):
    ...
    def test_syn_uses_project_target(self, run) -> None:
        ...
        def create_outputs(command, **kwargs):
            if command[1] == "syn-artifacts":
                contract.update(self.write_syn_contract(top="uart", syn_tool="yosys"))
            elif command[1] == "syn":
    ...
    def test_formal_accepts_plain_dc_snapshot(self, run) -> None:
        ...
        result = SERVER.soc_formal(str(self.module_dir), run_id, fingerprint, rtl_top="demo", timeout=60)
    ...
    def test_formal_accepts_upf_dc_snapshot(self, run) -> None:
        ...
        self.assertIn(f"FORMAL_REFERENCE_UPF={native / 'upf' / 'demo.upf'}", command)
        self.assertIn(
    ...
    def test_formal_rejects_modified_snapshot_artifact(self) -> None:
        run_id, fingerprint, native = self.write_syn_snapshot(upf=False)
        (native / "demo_netlist.v").write_text("module demo; wire modified; endmodule\n", encoding="utf-8")
    ...
    def test_syn_evidence_captures_upf_and_svf(self, run) -> None:
        ...
        def create_outputs(command, **kwargs):
            if command[1] == "syn-artifacts":
                contract.update(self.write_syn_contract(top="uart", syn_tool="dc", upf=True))
                transient = (self.module_dir / "de" / "syn" / "dc" / "reports" / "loaded_upf.pre_compile.upf")
        ...
        manifest = [item for item in evidence["artifacts"] if item.endswith("artifact_manifest.json")]
    ...
    def test_syn_evidence_always_captures_canonical_rtl_f(self, run) -> None:
        ...
        def create_outputs(command, **kwargs):
            if command[1] == "syn-artifacts":
                ...
                contract["rtl_filelist"].write_text(rtl_f.read_text(encoding="utf-8"), encoding="utf-8")
            elif command[1] == "syn":
                contract["netlist"].write_text("module demo; endmodule\n", encoding="utf-8")
                contract["svf"].write_text("svf\n", encoding="utf-8")
            return "ok"
    ...
    def test_syn_rejects_missing_declared_reference_upf(self, run) -> None:
        ...
        def declare_missing_reference(command, **kwargs):
            if command[1] == "syn-artifacts":
                contract.update(self.write_syn_contract(top="demo", syn_tool="dc", upf=True))
                contract["reference_upf"].unlink()
            return "ok"

        run.side_effect = declare_missing_reference
        with self.assertRaisesRegex(RuntimeError, "canonical reference UPF is missing or empty"):
            SERVER.soc_syn(str(self.module_dir), "demo", syn_tool="dc")
    ...
    def test_syn_rejects_reference_upf_mutation(self, run) -> None:
        ...
        def mutate_reference(command, **kwargs):
            if command[1] == "syn-artifacts":
                ...
                self.write_syn_products(contract)
                contract["reference_upf"].write_text("set_design_top changed\n", encoding="utf-8")
            return "ok"
    ...
    def test_syn_rejects_derived_logic_filelist(self, run) -> None:
        ...
        def create_filtered_outputs(command, **kwargs):
            if command[1] == "syn-artifacts":
                ...
                self.write_syn_products(contract)
                contract["rtl_filelist"].write_text("/different/logic_only.sv\n", encoding="utf-8")
            return "ok"
    ...
    def test_success_evidence_rejects_source_drift(self, run) -> None:
        ...
        def mutate(*args, **kwargs):
            if run.call_count == 2:
                (self.module_dir / "de" / "rtl" / "demo.sv").write_text("module demo; wire changed; endmodule\n")
            return "ok"
    ...
    def test_verdi_uses_scope(self, run) -> None:
        SERVER.soc_verdi(str(self.module_dir), scope="dv", simulator="vcs", top_module="tb", test="smoke")
        run.assert_called_once_with(
            [
                "make",
                "verdi",
    ...
    def test_coverage_regress(self, run) -> None:
        SERVER.soc_coverage(str(self.module_dir), "vcs", "regress", tests="uart_all", seeds="1-2", jobs=2)
        run.assert_called_once_with(
            [
                "make",
                "coverage-regress",
