...
def create_ip_template(root: Path, ip_name: str, project_name: str, ip_type: str = "digital"):
    ...
    (ip_dir / "de" / "rtl" / "filelist.f").write_text(f"$SOC/ip/{ip_type}/{ip_name}/de/rtl/{ip_name}.v\n",
                                                      encoding="utf-8")
...
def init_project(project_name, output_path, top_module=None):
    ...
    for sub in CHIP_SUBMODULES:
        ...
        (root / "chip" / sub / "Makefile").write_text(_module_makefile(module_name, 2, rtl_entry=sub == "top"),
                                                      encoding="utf-8")
        (root / "chip" / sub / "README.md").write_text(CHIP_SUB_README_MD.format(**sub_fmt), encoding="utf-8")
        (root / "chip" / sub / "dv" / "tb" / f"tb_{module_name}.sv").write_text(CHIP_SUB_TB_SV.format(**sub_fmt),
                                                                                encoding="utf-8")
        ...
        (root / "chip" / sub / "de" / "rtl" / "filelist.f").write_text(f"$SOC/chip/{sub}/de/rtl/{module_name}.v\n",
                                                                       encoding="utf-8")
...
def add_chip_module(module_name, project_path):
    ...
    (module_dir / "de" / "rtl" / f"{module_name}.v").write_text(CHIP_SUB_RTL_V.format(**fmt), encoding="utf-8")
    (module_dir / "de" / "rtl" / "filelist.f").write_text(f"$SOC/chip/{module_name}/de/rtl/{module_name}.v\n",
                                                          encoding="utf-8")
...
def main():
    ...
    addip_parser.add_argument("-t",
                              "--type",
                              default="digital",
                              choices=["digital", "third_party"],
