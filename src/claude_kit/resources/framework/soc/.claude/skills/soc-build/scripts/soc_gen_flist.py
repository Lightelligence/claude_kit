...
def gen_flist(search_path, output="filelist.f", recursive=True, extensions=None):
    ...
    label = "stdout" if output == "-" else Path(output).name
    content = (f"# Auto-generated filelist: {label}\n"
               f"# Source path: {path}\n"
...
def main():
    parser = argparse.ArgumentParser(description="Generate Verilog/SystemVerilog filelist (.f)")
