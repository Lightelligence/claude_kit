...
mcp = FastMCP(
    name="soc-build",
    instructions=("SoC 项目脚手架和 EDA Make 目标执行工具。\n"
...
def _make(
    ...
    if os.environ.get(MCP_SERVER_ACTIVE_ENV) != "1":
        raise RuntimeError("EDA Make targets must run through the registered soc-build MCP server; "
...
def _native_evidence_files(path: Path, tool_family: str) -> list[Path]:
    ...
    patterns = (("dv/sim/**/sim.log", "dv/sim/**/*.log") if tool_family == "soc_sim" else (
        "de/syn/**/*.log",
        "de/syn/**/*.rpt",
        "de/syn/**/*netlist*.v",
        "de/syn/**/*netlist*.sv",
...
def _artifact_signatures(paths: list[Path]) -> dict[Path, tuple[int, int]]:
    return {
        ...
        for path in paths if path.is_file() and (stat := path.stat()).st_size > 0
    }
...
def _read_syn_artifact_contract(path: Path, syn_tool: str) -> dict[str, Path]:
    ...
    if missing:
        raise RuntimeError("synthesis artifact manifest is missing keys: " + ", ".join(sorted(missing)))
    required = {"RTL_FILELIST", "NETLIST"}
    if syn_tool == "dc":
        required.add("SVF")
    empty = sorted(key for key in required if not values[key])
    if empty:
        raise RuntimeError("synthesis artifact manifest has empty required keys: " + ", ".join(empty))
    if bool(values["REFERENCE_UPF"]) != bool(values["IMPLEMENTATION_UPF"]):
        raise RuntimeError("synthesis artifact manifest must pair REFERENCE_UPF and IMPLEMENTATION_UPF")
    ...
    for key, label in SYN_ARTIFACT_KEYS.items():
        ...
        try:
            ...
            raise RuntimeError(f"synthesis artifact {key} escapes module directory: {candidate}") from exc
        contract[label] = candidate
    return contract


def _validate_syn_artifact_contract(contract: dict[str, Path]) -> None:
    missing = [f"{label}={path}" for label, path in contract.items() if not path.is_file() or path.stat().st_size == 0]
    if missing:
        raise RuntimeError("synthesis did not produce required nonempty artifacts: " + ", ".join(missing))
...
def _capture_loop_artifacts(
    ...
    if tool_family == "soc_syn":
        ...
        candidates = [candidate for candidate in candidates if candidate.suffix in {".log", ".rpt"}]
        candidates.extend(syn_artifacts.values())
        source_inputs = {
            syn_artifacts["rtl_filelist"],
            *([syn_artifacts["reference_upf"]] if "reference_upf" in syn_artifacts else []),
        }
    ...
    for candidate in sorted(set(candidates)):
        ...
        if stat.st_size <= 0 or (before.get(candidate) == signature and not source_input):
            continue
        relative = candidate.relative_to(path)
    ...
    if tool_family == "soc_syn":
        ...
        for label, source in syn_artifacts.items():
            ...
            if destination is None:
                raise RuntimeError(f"synthesis artifact was not captured as fresh evidence: {label}={source}")
...
def _make_with_loop_evidence(
    ...
    if tool_family == "soc_syn":
        ...
        syn_artifacts = _read_syn_artifact_contract(path, str(variables.get("SYN_TOOL", "yosys")))
        ...
        reference_upf_digest = (_sha256(syn_artifacts["reference_upf"])
                                if "reference_upf" in syn_artifacts and syn_artifacts["reference_upf"].is_file()
                                and syn_artifacts["reference_upf"].stat().st_size > 0 else None)
        if "reference_upf" in syn_artifacts and reference_upf_digest is None:
            raise RuntimeError("declared canonical reference UPF is missing or empty: "
                               f"{syn_artifacts['reference_upf']}")
    source_fingerprint = compute_rtl_fingerprint(path)
    if not source_fingerprint:
        raise RuntimeError("cannot create loop evidence: resolved RTL/filelist fingerprint is empty")
    ...
    if syn_artifacts:
        ...
        if _sha256(syn_artifacts["rtl_filelist"]) != _sha256(canonical_rtl):
            raise RuntimeError("synthesis rtl.f differs from the canonical de/run/rtl.f; "
                               "do not derive or filter a second logic filelist")
        if (reference_upf_digest is not None and _sha256(syn_artifacts["reference_upf"]) != reference_upf_digest):
    ...
    if final_fingerprint != source_fingerprint:
        raise RuntimeError(f"RTL/filelist changed during {tool_family}; discard this run and rerun")
    evidence = {
        "run_id":
        run_id,
        "source_fingerprint":
    ...
    return str(output).rstrip() + "\nLOOP_EVIDENCE=" + json.dumps(evidence, sort_keys=True)
...
def _unique_artifact(paths: list[Path], label: str) -> Path:
    ...
    if len(candidates) != 1:
        raise RuntimeError(f"registered synthesis evidence must contain exactly one {label}; "
...
def _resolve_formal_snapshot(
    ...
    if current_fingerprint != syn_source_fingerprint:
        raise RuntimeError("current RTL/filelist fingerprint does not match the registered "
    ...
    if not manifest_path.is_file() or manifest_path.stat().st_size == 0:
        raise RuntimeError(f"registered synthesis artifact manifest is missing: {manifest_path}")
    try:
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise RuntimeError(f"registered synthesis artifact manifest is invalid: {manifest_path}") from exc
    if (manifest.get("version") != 1 or manifest.get("run_id") != syn_run_id
            or manifest.get("source_fingerprint") != syn_source_fingerprint):
        raise RuntimeError("registered synthesis artifact manifest identity does not match "
    ...
    if missing:
        raise RuntimeError("registered synthesis artifact manifest is missing: " + ", ".join(missing))
    ...
    if has_reference != has_implementation:
        raise RuntimeError("registered synthesis artifact manifest must pair reference and "
                           "implementation UPF")
    allowed = required | {"reference_upf", "implementation_upf"}
    unknown = sorted(set(entries) - allowed)
    if unknown:
        raise RuntimeError("registered synthesis artifact manifest has unknown entries: " + ", ".join(unknown))
    ...
    for label, entry in entries.items():
        ...
        try:
            ...
            raise RuntimeError(f"synthesis artifact path escapes evidence directory: {artifact}") from exc
...
def _validate_formal_reports(artifacts: list[Path], use_upf: bool) -> None:
    ...
    status = by_name["verification_status.rpt"].read_text(encoding="utf-8", errors="replace")
    required = {
        "Status:             SUCCEEDED",
        "Failing Points:     0",
        "Aborted Points:     0",
    ...
    defects = by_name["library_defects.rpt"].read_text(encoding="utf-8", errors="replace")
    ...
    if use_upf and re.search(r"partial verification result|all UPF supplies.*ON state", log, re.I):
...
def _run_registered_formal(
    ...
    snapshot, use_upf = _resolve_formal_snapshot(module, syn_run_id, syn_source_fingerprint)
    ...
    stale = [str(path) for path in artifacts if path in before and path in after and before[path] == after[path]]
    if missing:
    ...
    return output.rstrip() + "\nFORMAL_PASS\nFORMAL_EVIDENCE=" + json.dumps(evidence, sort_keys=True)
...
def soc_flist(path: str, output: str = "", recursive: bool = False) -> str:
    ...
    Args:
        ...
        recursive: 是否递归扫描子目录
    """
    args = [path, "-o", output or "-", "-r" if recursive else "-n"]
