...
def _run_generator(script, *arguments, cwd=None):
    """Use the active interpreter and propagate child failures to the MCP caller."""
    import subprocess
    subprocess.run([sys.executable, script, *arguments], cwd=cwd, check=True)


def _crg_register_csv_contract(lines, design_name, protocol):
    """Match the embedded APB regfile without changing SDC instance paths."""
    if protocol != "apb":
        return lines
    legacy = design_name.upper() + "_apb_reg"
    module = legacy + "file"
    ports = {"clk": "apb_clk", "rst_n": "apb_rst_n",
             "psel": "apb_sel", "penable": "apb_enable",
             "pwrite": "apb_write", "paddr": "apb_addr",
             "pwdata": "apb_wdata", "prdata": "apb_rdata",
             "pready": "apb_ready", "pslverr": "apb_slverr"}
    result = []
    for line in lines:
        if line.startswith("inst " + legacy + " "):
            line = "inst " + module + line[len("inst " + legacy):]
        elif line.startswith("connect,"):
            parts = line.split(",")
            owner, separator, port = parts[1].partition(".")
            if separator and owner in {legacy, module}:
                parts[1] = module + "." + ports.get(port.strip(), port.strip())
                line = ",".join(parts)
        result.append(line)
    return result
...
def main():
    ...
    if crg_option == "sdc_gen" :
        ...
        _run_generator(soc_build_py, "gen",
                       os.path.join(gen_filepath, top_corpus[top_info_index["design_name"]][1] + "_top.csv"))
...
def clk_sdc_gen(rst_info_index, clk_info_index, top_info_index, top_corpus, clk_corpus, clk_ser, rst_corpus, rst_ser,
                sdc_corpus, sdc_ser, sdc_empty):
    ...
    for clk_info in clk_corpus:
        if "source" in str(clk_info[clk_info_index["clock_source"]]) and pd.isna(
                clk_info[clk_info_index["div"]]) == True and pd.isna(
                    clk_info[clk_info_index["sel"]]) == True and pd.isna(clk_info[clk_info_index["icg"]]) == True and (
                        str(clk_info[clk_info_index["attr"]]) == "input"
                        or str(clk_info[clk_info_index["attr"]]) == "internal"):
            ...
            if pd.isna(clk_info[clk_info_index["sel"]]) == False:
                for clk_source_info in clock_source_list:
                    ...
                    if clk_info[clk_info_index["src0"]] in clk_source_info:
                        ...
                        clock_generate_list.append(clk_info[clk_info_index["name"]] + ", get_pins {" + design_hier +
                                                   "u_" + top_corpus[top_info_index["design_name"]][1] + "_clk_gen/u_" +
                                                   clk_info[clk_info_index["name"]] +
                                                   "_clk_glitch_free_switch/u_clk_out/Z}")
                ...
                for clk_source_info in clock_source_list:
                    ...
                    if clk_info[clk_info_index["src0"]] in clk_source_info:
                        ...
                        clock_generate_list.append(
                            clk_info[clk_info_index["name"]] + ", get_pins {" + design_hier + "u_" +
                            top_corpus[top_info_index["design_name"]][1] + "_clk_gen/u_" +
                            clk_info[clk_info_index["name"]] +
                            "_divider_wrap/u_clk_divider/BYPASS0*u_std_cell_clk_buf/u_std_cell_clk_buf/Z}")
                ...
                for clk_source_info in clock_source_list:
                    ...
                    if clk_info[clk_info_index["src0"]] in clk_source_info:
                        ...
                        clock_generate_list.append(clk_info[clk_info_index["name"]] + ", get_pins {" + design_hier +
                                                   "u_" + top_corpus[top_info_index["design_name"]][1] + "_clk_gen/u_" +
                                                   clk_info[clk_info_index["name"]] + "_icg/u_dontouch_icg/Q}")
                ...
                for clk_source_info in sdc_constrain_source:
                    ...
                    if clk_info[clk_info_index["src0"]] in clk_source_info:
                        ...
                        if master_clock_flag == 1:
                            sdc_all_clk_source.append(clk_info[clk_info_index["name"]] + ", get_pins {" + design_hier +
                                                      "u_" + top_corpus[top_info_index["design_name"]][1] +
                                                      "_clk_gen/u_" + clk_info[clk_info_index["name"]] +
                                                      "_clk_glitch_free_switch/u_clk_out/Z}")
                        else:
                            sdc_all_clk_source.append(
                                str(clk_source_name.replace(clk_info[clk_info_index["src0"]], "")) +
                                clk_info[clk_info_index["name"]] + ", get_pins {" + design_hier + "u_" +
                                top_corpus[top_info_index["design_name"]][1] + "_clk_gen/u_" +
                                clk_info[clk_info_index["name"]] + "_clk_glitch_free_switch/u_clk_out/Z}")
                    #if clk_info[clk_info_index["src1"]] in clk_source_info :
                    #    clk_source_temp = clk_source_info.split(",")
                    #    clk_source_name = clk_source_temp[0]
                    #    clk_source_path = clk_source_temp[1]
    ...
    for clk_info in clk_corpus:
        if "source" in str(clk_info[clk_info_index["clock_source"]]) and pd.isna(
            ...
            print_line.append("# ----------------------------------------------------------------------")
            print_line.append("# create clock for " + clk_info[clk_info_index["name"]] + " frequency: " +
                              clk_info[clk_info_index["note"]])
            print_line.append("create_clock -name " + clk_info[clk_info_index["name"]] + " -period " +
                              str(int(clock_freq * 10000) / 10000) + " -waveform {0.000 " +
                              str(int(clock_freq_half * 10000) / 10000) + "} [" + clock_source + "] -add")
    ...
    if sdc_empty == False:
        for sdc_info in sdc_corpus:
            if sdc_info[1] == "P":
                ...
                clock_freq_half = clock_freq / 2
                #print_line.append("create_clock -name "+clk_info[clk_info_index["name"]]+" -period "+str(format(clock_freq, ".4f"))+" -waveform {0.000 "+str(format(clock_freq_half, ".4f"))+"} ["+clock_source+"] -add")
                print_line.append("create_clock -name " + sdc_info[0] + " -period " +
                                  str(int(clock_freq * 10000) / 10000) + " -waveform {0.000 " +
                                  str(int(clock_freq_half * 10000) / 10000) + "} [" + sdc_info[3] + "] -add")
            ...
            if pd.isna(clk_info[clk_info_index["sel"]]) == False:
                    ...
                    if clk_info[clk_info_index["src0"]] in clk_source_info:
                        ...
                        for master_clock in sdc_all_clk_source:
                            if clk_source_name in master_clock:
                                master_clock_flag = 1
                        if master_clock_flag == 1:
                            print_line.append("# generated clock for " + clk_info[clk_info_index["name"]] +
                                              " from clk_sel frequency: " + clk_info[clk_info_index["note"]])
                            print_line.append("create_generated_clock -name " + clk_info[clk_info_index["name"]] +
                                              " -combinational \\")
                            all_clock.append(clk_info[clk_info_index["name"]])
                        else:
                            print_line.append("# generated clock for " +
                                              str(clk_source_name.replace(clk_info[clk_info_index["src0"]], "")) +
                                              clk_info[clk_info_index["name"]] + " from clk_sel frequency: " +
                                              clk_info[clk_info_index["note"]])
                            print_line.append("create_generated_clock -name " +
                                              str(clk_source_name.replace(clk_info[clk_info_index["src0"]], "")) +
                                              clk_info[clk_info_index["name"]] + " -combinational \\")
                            all_clock.append(
                                str(clk_source_name.replace(clk_info[clk_info_index["src0"]], "")) +
                                clk_info[clk_info_index["name"]])
                        ...
                        print_line.append("     -edges {1 2 3} [get_pins {" + design_hier + "u_" +
                                          top_corpus[top_info_index["design_name"]][1] + "_clk_gen/u_" +
                                          clk_info[clk_info_index["name"]] + "_clk_glitch_free_switch/u_clk_out/Z}] \\")
                        #for master_clock in sdc_all_clk_source :
                        #    if clk_source_name in master_clock :
                        ...
                        for master_clock in sdc_all_clk_source:
                            if clk_source_name in master_clock:
                                master_clock_flag = 1
                        if master_clock_flag == 1:
                            set_clk_info(clk_info_index, top_info_index, print_line, top_corpus,
                                         clk_info[clk_info_index["name"]])
                        else:
                            set_clk_info(
                                clk_info_index, top_info_index, print_line, top_corpus,
                                str(clk_source_name.replace(clk_info[clk_info_index["src0"]], "")) +
                                clk_info[clk_info_index["name"]])
                        print_line.append("set_case_analysis 0 " + design_hier + "u_" +
                                          top_corpus[top_info_index["design_name"]][1] + "_clk_gen/u_" +
                                          clk_info[clk_info_index["name"]] + "_clk_glitch_free_switch/sel")
                ...
                for clk_source_info in sdc_all_clk_source:
                    ...
                    if clk_info[clk_info_index["src1"]] in clk_source_info:
                        ...
                        if master_clock_flag == 1:
                            ...
                            print_line.append("# generated clock for " +
                                              str(clk_source_name.replace(clk_info[clk_info_index["src1"]], "")) +
                                              clk_info[clk_info_index["name"]] + " from clk_sel frequency: " +
                                              clk_info[clk_info_index["note"]])
                            print_line.append("create_generated_clock -name " +
                                              str(clk_source_name.replace(clk_info[clk_info_index["src1"]], "")) +
                        ...
                        if master_clock_flag == 1:
                            ...
                            set_clk_info(
                                ...
                                str(clk_source_name.replace(clk_info[clk_info_index["src1"]], "")) +
                                clk_info[clk_info_index["name"]])
                        print_line.append("set_case_analysis 1 " + design_hier + "u_" +
                                          top_corpus[top_info_index["design_name"]][1] + "_clk_gen/u_" +
                                          clk_info[clk_info_index["name"]] + "_clk_glitch_free_switch/sel")
                ...
                for clk_source_info in sdc_all_clk_source:
                    ...
                    if clk_info[clk_info_index["src0"]] in clk_source_info:
                        ...
                        if master_clock_flag == 1:
                            print_line.append("# generated clock for " + clk_info[clk_info_index["name"]] +
                                              " default: div" + str(int(clk_info[clk_info_index["div_dflt"]])) +
                                              " frequency: " + clk_info[clk_info_index["note"]])
                            print_line.append("create_generated_clock -name " + clk_info[clk_info_index["name"]] +
                                              " \\")
                            all_clock.append(clk_info[clk_info_index["name"]])
                        else:
                            print_line.append("# generated clock for " +
                                              str(clk_source_name.replace(clk_info[clk_info_index["src0"]], "")) +
                                              clk_info[clk_info_index["name"]] + " default: div" +
                                              str(int(clk_info[clk_info_index["div_dflt"]])) + "frequency: " +
                                              clk_info[clk_info_index["note"]])
                            print_line.append("create_generated_clock -name " +
                                              str(clk_source_name.replace(clk_info[clk_info_index["src0"]], "")) +
                                              clk_info[clk_info_index["name"]] + " \\")
                        ...
                        clock_source = clk_source_path.strip("source ")
                        print_line.append("     -source [" + clock_source + "] \\")
                        if (int(clk_info[clk_info_index["div_dflt"]] %
                                2)) == 0 or clk_info[clk_info_index["div_dflt"]] == 1:
                            print_line.append("     -edges {1 " + str(int(clk_info[clk_info_index["div_dflt"]] + 1)) +
                                              " " + str(int(clk_info[clk_info_index["div_dflt"]] * 2 + 1)) + "} \\")
                        else:
                            print_line.append("     -edges {1 " + str(int(clk_info[clk_info_index["div_dflt"]])) + " " +
                                              str(int(clk_info[clk_info_index["div_dflt"]] * 2 + 1)) + "} \\")
                        print_line.append(
                            "     [get_pins {" + design_hier + "u_" + top_corpus[top_info_index["design_name"]][1] +
                            "_clk_gen/u_" + clk_info[clk_info_index["name"]] +
                            "_divider_wrap/u_clk_divider/BYPASS0*u_std_cell_clk_buf/u_std_cell_clk_buf/Z}] \\")
                        print_line.append("     -master_clock [get_clocks {" + clk_source_name + "}] -add")
                        master_clock_flag = 0
                ...
                for clk_source_info in sdc_all_clk_source:
                    ...
                    if clk_info[clk_info_index["src0"]] in clk_source_info:
                        ...
                        if master_clock_flag == 1:
                            print_line.append("# generated clock for " + clk_info[clk_info_index["name"]] +
                                              " icg frequency: " + clk_info[clk_info_index["note"]])
                            print_line.append("create_generated_clock -name " + clk_info[clk_info_index["name"]] +
                                              " -combinational \\")
                            all_clock.append(clk_info[clk_info_index["name"]])
                        else:
                            print_line.append("# generated clock for " +
                                              str(clk_source_name.replace(clk_info[clk_info_index["src0"]], "")) +
                                              clk_info[clk_info_index["name"]] + " icg frequency: " +
                                              clk_info[clk_info_index["note"]])
                            print_line.append("create_generated_clock -name " +
                                              str(clk_source_name.replace(clk_info[clk_info_index["src0"]], "")) +
                                              clk_info[clk_info_index["name"]] + " -combinational \\")
    ...
    if sdc_empty == False:
        for sdc_info in sdc_corpus:
            if sdc_info[1] == "G":
                print_line.append("# ----------------------------------------------------------------------")
                print_line.append("# generated clock for " + sdc_info[0] + " frequency: " +
                                  clk_info[clk_info_index["note"]])
    ...
    print_line.append("set_false_path -through [get_pins -of_objects {" + design_hier + "u_" +
                      top_corpus[top_info_index["design_name"]][1].upper() +
                      "_apb_reg} -filter \"direction==in && full_name=~*rst_n_status\"] ")
    print_line.append("set_false_path -through [get_pins -of_objects {" + design_hier + "u_" +
                      top_corpus[top_info_index["design_name"]][1].upper() +
                      "_apb_reg} -filter \"direction==in && full_name=~*clk_divider_status[*]\"]")
    print_line.append("set_false_path -through [get_pins -of_objects {" + design_hier + "u_" +
                      top_corpus[top_info_index["design_name"]][1].upper() +
                      "_apb_reg} -filter \"direction==out && full_name=~*clk_ea\"]")
    print_line.append("set_false_path -through [get_pins -of_objects {" + design_hier + "u_" +
                      top_corpus[top_info_index["design_name"]][1].upper() +
                      "_apb_reg} -filter \"direction==in && full_name=~*clk_ea_status\"]")
    print_line.append("set_false_path -through [get_pins -of_objects {" + design_hier + "u_" +
                      top_corpus[top_info_index["design_name"]][1].upper() +
                      "_apb_reg} -filter \"direction==in && full_name=~*clk0_sel\"]")
    print_line.append("set_false_path -through [get_pins -of_objects {" + design_hier + "u_" +
                      top_corpus[top_info_index["design_name"]][1].upper() +
                      "_apb_reg} -filter \"direction==in && full_name=~*clk1_sel\"]")
    print_line.append("set_false_path -through [get_pins -of_objects {" + design_hier + "u_" +
                      top_corpus[top_info_index["design_name"]][1].upper() +
                      "_apb_reg} -filter \"direction==in && full_name=~*sel_done\"]")
    ...
    for rst_info in rst_corpus:
        if rst_info[rst_info_index["areset_relax_en"]] == "Y":
            print_line.append("set_multicycle_path -setup 2 -from [get_pins " + design_hier + "u_" +
                              top_corpus[top_info_index["design_name"]][1] + "_rst_gen/u_" +
                              rst_info[rst_info_index["name"]] + "_test_mux/rstn_out]")
            print_line.append("set_multicycle_path -hold 1 -from [get_pins " + design_hier + "u_" +
                              top_corpus[top_info_index["design_name"]][1] + "_rst_gen/u_" +
                              rst_info[rst_info_index["name"]] + "_test_mux/rstn_out]")
    print_line.append("")
    print_line.append("")
    ...
    for clk_info in clk_corpus_group0:
        if (pd.isna(clk_info[clk_info_index["sel"]]) == False or pd.isna(clk_info[clk_info_index["div"]]) == False
                or "source" in str(clk_info[clk_info_index["clock_source"]])
                or clk_info[clk_info_index["icg"]] == "Y") and clk_info[clk_info_index["name"]] != "nan":
            #print("################")
            #print(clk_info[clk_info_index["name"]]+","+clk_info[clk_info_index["clock_group0"]])
            if count == 0:
                print_line.append("set_clock_groups -name " + top_corpus[top_info_index["design_name"]][1] +
                                  "_group -asynchronous \\")
            ...
            #print(clk_group)
            for clk_info in clk_corpus_group0:
                if pd.isna(clk_info[clk_info_index["sel"]]) == False or pd.isna(
                        clk_info[clk_info_index["div"]]) == False or "source" in str(
                            clk_info[clk_info_index["clock_source"]]) or clk_info[clk_info_index["icg"]] == "Y":
                    if clk_info[clk_info_index["clock_group0"]] == clk_group and clk_name != clk_info[
                            clk_info_index["name"]]: #and clk_info[clk_info_index["name"]] not in  print_line[-1] :
...
def crg_gen_xml(rst_info_index, gen_filepath, top_corpus, rst_corpus, rst_ser, clk_info_index, top_info_index,
                clk_corpus, clk_ser, reg_corpus, reg_ser, reg_empty, intp_empty, intp_corpus, intp_ser): #{{{
    ...
    print_line.append("    <spirit:name>" + top_corpus[top_info_index["design_name"]][1].upper() + "</spirit:name>")
    print_line.append("    <spirit:description>" + top_corpus[top_info_index["design_name"]][1].upper() +
                      " regfile</spirit:description>")
    ...
    clk_sheet_cnt = 0
    for clk_info in clk_corpus:
        if clk_info[clk_info_index["attr"]] == "output" or clk_info[clk_info_index["attr"]] == "na" or clk_info[
                clk_info_index["attr"]] == "internal":
            if clk_info[clk_info_index["icg"]] == "Y" or pd.isna(clk_info[clk_info_index["sel"]]) == False or pd.isna(
                    clk_info[clk_info_index["div"]]) == False:
                ...
                print_line.append("      <spirit:description>" + clk_info[clk_info_index["name"]] +
                                  " control register</spirit:description>")
                ...
                print_line.append("        <spirit:value>" +
                                  str(hex(int((clk_ctrl_bit_high + clk_ctrl_bit8 + clk_ctrl_bit0), 16))) +
                                  "</spirit:value>")
                print_line.append("      </spirit:reset>")
                count += 1
            if clk_info[clk_info_index["icg"]] == "Y":
                ...
                print_line.append("        <spirit:description>" + clk_info[clk_info_index["name"]] +
                                  " icg enable, PRTC_16_16_1'h1</spirit:description>")
            ...
            if pd.isna(clk_info[clk_info_index["div"]]) == False:
                print_line.append("      <spirit:field>")
                print_line.append("        <spirit:name>" + clk_info[clk_info_index["name"]] +
                                  "_divider_ea_req</spirit:name>")
                print_line.append("        <spirit:description>" + clk_info[clk_info_index["name"]] +
                                  " divider enable request, PRTC_20_20_1'h1</spirit:description>")
            ...
            if pd.isna(clk_info[clk_info_index["sel"]]) == False:
                ...
                print_line.append("        <spirit:description>" + clk_info[clk_info_index["name"]] +
                                  " select, PRTC_24_24_1'h1</spirit:description>")
            ...
            if clk_info[clk_info_index["icg"]] == "Y":
                print_line.append("      <spirit:field>")
                print_line.append("        <spirit:name>" + clk_info[clk_info_index["name"]] +
                                  "_ea_lock_fld</spirit:name>")
                print_line.append("        <spirit:description>" + clk_info[clk_info_index["name"]] +
                                  " icg enable lock, PRTC_16_16_1'h1</spirit:description>")
            ...
            if pd.isna(clk_info[clk_info_index["div"]]) == False:
                print_line.append("      <spirit:field>")
                print_line.append("        <spirit:name>" + clk_info[clk_info_index["name"]] +
                                  "_divider_ea_req_lock_fld</spirit:name>")
                print_line.append("        <spirit:description>" + clk_info[clk_info_index["name"]] +
                                  " divider enable request lock, PRTC_20_20_1'h1</spirit:description>")
            ...
            if pd.isna(clk_info[clk_info_index["sel"]]) == False:
                print_line.append("      <spirit:field>")
                print_line.append("        <spirit:name>" + clk_info[clk_info_index["name"]] +
                                  "_sel_lock_fld</spirit:name>")
                print_line.append("        <spirit:description>" + clk_info[clk_info_index["name"]] +
                                  " select lock, PRTC_24_24_1'h1</spirit:description>")
            ...
            if pd.isna(clk_info[clk_info_index["div"]]) == False:
                ...
                print_line.append("      <spirit:description>" + clk_info[clk_info_index["name"]] +
                                  " divider</spirit:description>")
                print_line.append("      <spirit:addressOffset>" + reg_addr + "</spirit:addressOffset>")
                print_line.append("      <spirit:size>32</spirit:size>")
                print_line.append("      <spirit:access>RW</spirit:access>")
                print_line.append("      <spirit:reset>")
                print_line.append("        <spirit:value>" + str(hex(int(clk_info[clk_info_index["div_dflt"]]))) +
                                  "</spirit:value>")
                print_line.append("      </spirit:reset>")
                count += 1
            if pd.isna(clk_info[clk_info_index["div"]]) == False:
                ...
                print_line.append("        <spirit:name>" + clk_info[clk_info_index["name"]] + "_divider</spirit:name>")
                print_line.append("        <spirit:description>" + clk_info[clk_info_index["name"]] +
                                  " divider, PRTC_" + str(int(clk_info[clk_info_index["div_width"]] + 16)) + "_16_" +
                                  str(int(clk_info[clk_info_index["div_width"]])) + "'h" +
                                  str(hex(lock_value)).strip("0x") + "</spirit:description>")
                print_line.append("        <spirit:bitOffset>0</spirit:bitOffset>")
                print_line.append("        <spirit:bitWidth>" + str(int(clk_info[clk_info_index["div_width"]])) +
                                  "</spirit:bitWidth>")
                print_line.append("        <spirit:access>RW</spirit:access>")
                print_line.append("        <spirit:lockOffset>16</spirit:lockOffset>")
                print_line.append("        <spirit:lockWidth>" + str(int(clk_info[clk_info_index["div_width"]])) +
                                  "</spirit:lockWidth>")
                print_line.append("        <spirit:lockValue>" + str(hex(lock_value)) + "</spirit:lockValue>")
                print_line.append("      </spirit:field>")
                print_line.append("      <spirit:field>")
                print_line.append("        <spirit:name>" + clk_info[clk_info_index["name"]] +
                                  "_divider_lock_fld</spirit:name>")
                print_line.append("        <spirit:description>" + clk_info[clk_info_index["name"]] +
                                  " divider lock, PRTC_" + str(int(clk_info[clk_info_index["div_width"]] + 16)) +
                                  "_16_" + str(int(clk_info[clk_info_index["div_width"]])) + "'h" +
                                  str(hex(lock_value)).strip("0x") + "</spirit:description>")
                print_line.append("        <spirit:bitOffset>16</spirit:bitOffset>")
                print_line.append("        <spirit:bitWidth>" + str(int(clk_info[clk_info_index["div_width"]])) +
                                  "</spirit:bitWidth>")
            ...
            if clk_info[clk_info_index["icg"]] == "Y":
                print_line.append("      <spirit:field>")
                print_line.append("        <spirit:name>" + clk_info[clk_info_index["name"]] +
                                  "_ea_status</spirit:name>")
                print_line.append("        <spirit:description>" + clk_info[clk_info_index["name"]] +
                                  " icg enable status</spirit:description>")
                print_line.append("        <spirit:bitOffset>0</spirit:bitOffset>")
                print_line.append("        <spirit:bitWidth>1</spirit:bitWidth>")
                print_line.append("        <spirit:access>RO</spirit:access>")
                print_line.append("      </spirit:field>")
            if pd.isna(clk_info[clk_info_index["sel"]]) == False:
                print_line.append("      <spirit:field>")
                print_line.append("        <spirit:name>" + clk_info[clk_info_index["name"]] +
                                  "_sel_clk0_sel</spirit:name>")
                print_line.append("        <spirit:description>" + clk_info[clk_info_index["name"]] +
                                  " select clk1 status</spirit:description>")
                print_line.append("        <spirit:bitOffset>8</spirit:bitOffset>")
                print_line.append("        <spirit:bitWidth>1</spirit:bitWidth>")
                print_line.append("        <spirit:access>RO</spirit:access>")
                print_line.append("      </spirit:field>")
                ...
                print_line.append("      <spirit:field>")
                print_line.append("        <spirit:name>" + clk_info[clk_info_index["name"]] +
                                  "_sel_done</spirit:name>")
                print_line.append("        <spirit:description>" + clk_info[clk_info_index["name"]] +
                                  " select done status</spirit:description>")
            ...
            if pd.isna(clk_info[clk_info_index["div"]]) == False:
                print_line.append("      <spirit:field>")
                print_line.append("        <spirit:name>" + clk_info[clk_info_index["name"]] +
                                  "_divider_done</spirit:name>")
                print_line.append("        <spirit:description>" + clk_info[clk_info_index["name"]] +
                                  " divider done status</spirit:description>")
                ...
                print_line.append("      <spirit:field>")
                print_line.append("        <spirit:name>" + clk_info[clk_info_index["name"]] +
                                  "_divider_status</spirit:name>")
                print_line.append("        <spirit:description>" + clk_info[clk_info_index["name"]] +
                                  " divider status</spirit:description>")
                print_line.append("        <spirit:bitOffset>16</spirit:bitOffset>")
                print_line.append("        <spirit:bitWidth>" + str(int(clk_info[clk_info_index["div_width"]])) +
                                  "</spirit:bitWidth>")
                print_line.append("        <spirit:access>RO</spirit:access>")
    ...
    rst_reg_bit_lc = []
    for rst_info in rst_corpus:
        if pd.isna(rst_info[rst_info_index["soft_lc"]]) == True or pd.isna(
                rst_info[rst_info_index["soft_src"]]) == True:
            continue
        ...
        for rst_info in rst_corpus:
            ...
            if pd.isna(rst_info[rst_info_index["soft_lc"]]) == True or pd.isna(
                ...
                if rst_info_reg_lc[0] == rst_reg_num[int(count)]:
                    ...
                    #if rst_info[rst_info_index["soft_dflt"]] == "N" :
                    print_line.append("      <spirit:field>")
                    print_line.append("        <spirit:name>" + rst_info[rst_info_index["name"]] +
                                      "_sftrstn</spirit:name>")
                    #if pd.isna(rst_info[rst_info_index["lock_bit_offset"]]) == False :
                    print_line.append("        <spirit:description>" + rst_info[rst_info_index["name"]] +
                                      "_sftrstn, PRTC_" + str(int(rst_info_reg_lc[1]) + 16) + "_" +
                                      str(int(rst_info_reg_lc[1]) + 16) + "_0x1</spirit:description>")
                    #else :
                    ...
                    print_line.append("        <spirit:access>RW</spirit:access>")
                    #if pd.isna(rst_info[rst_info_index["lock_bit_offset"]]) == False :
                    print_line.append("        <spirit:lockOffset>" + str(int(rst_info_reg_lc[1]) + 16) +
                                      "</spirit:lockOffset>")
                    print_line.append("        <spirit:lockWidth>1</spirit:lockWidth>")
                    print_line.append("        <spirit:lockValue>0x1</spirit:lockValue>")
                    print_line.append("      </spirit:field>")
            if rst_info[rst_info_index[
                    "soft_src"]] == "SOFT": #and pd.isna(rst_info[rst_info_index["lock_bit_offset"]]) == False :
                if rst_info_reg_lc[0] == rst_reg_num[int(count)] and "        <spirit:name>" + rst_info[
                        rst_info_index["name"]] + "_lock_fld</spirit:name>" not in print_line:
                    print_line.append("      <spirit:field>")
                    print_line.append("        <spirit:name>" + rst_info[rst_info_index["name"]] +
                                      "_lock_fld</spirit:name>")
                    print_line.append("        <spirit:description>" + rst_info[rst_info_index["name"]] +
                                      " lock, PRTC_" + str(int(rst_info_reg_lc[1]) + 16) + "_" +
                                      str(int(rst_info_reg_lc[1]) + 16) + "_0x1</spirit:description>")
                    print_line.append("        <spirit:bitOffset>" + str(int(rst_info_reg_lc[1]) + 16) +
                                      "</spirit:bitOffset>")
                    print_line.append("        <spirit:bitWidth>1</spirit:bitWidth>")
                    print_line.append("        <spirit:access>W1T</spirit:access>")
                    print_line.append("      </spirit:field>")
...
#    print_line.append("blocks:")
    ...
    protocol = top_corpus[top_info_index["protocol"]][1]
    _run_generator(yml2reg_py, yml_file, protocol, cwd=gen_filepath)
...
def crg_gen_note(top_corpus, rst_corpus, rst_ser, clk_corpus, clk_ser, reg_corpus, reg_ser, reg_empty): #{{{
    ...
    if top_corpus[top_info_index["protocol"]][1] == "dab":
        print_line.append(top_corpus[top_info_index["design_name"]][1].upper() + "      --  0x0000:0xffff  --  " +
                          top_corpus[top_info_index["design_name"]][1].upper() + " regfile       -- dab")
    elif top_corpus[top_info_index["protocol"]][1] == "apb":
        print_line.append(top_corpus[top_info_index["design_name"]][1].upper() + "      --  0x0000:0xffff  --  " +
                          top_corpus[top_info_index["design_name"]][1].upper() + " regfile       -- apb")
    elif top_corpus[top_info_index["protocol"]][1] == "ahb":
        print_line.append(top_corpus[top_info_index["design_name"]][1].upper() + "      --  0x0000:0xffff  --  " +
                          top_corpus[top_info_index["design_name"]][1].upper() + " regfile       -- ahb")
    ...
    for clk_info in clk_corpus:
        if clk_info[clk_info_index["attr"]] == "output" or clk_info[clk_info_index["attr"]] == "na" or clk_info[
            ...
            if clk_info[clk_info_index["icg"]] == "Y" or pd.isna(clk_info[clk_info_index["sel"]]) == False or pd.isna(
                ...
                reg_addr = hex(count * 4)
                print_line.append("\n")
                print_line.append(reg_addr + "    			--		RW		--		" + clk_info[clk_info_index["name"]] +
                                  "_ctrl			    --		[31:0]		--	" + clk_info[clk_info_index["name"]] +
                                  " control register")
                count += 1
                #print(count)
            if clk_info[clk_info_index["icg"]] == "Y":
                if clk_info[clk_info_index["icg_dflt"]] == "Y":
                    print_line.append("--		'h1		--		RW		--		" + clk_info[clk_info_index["name"]] +
                                      "_ea		            --		[0]			--	" + clk_info[clk_info_index["name"]] +
                                      " icg enable")
                elif clk_info[clk_info_index["icg_dflt"]] == "N":
                    print_line.append("--		'h0		--		RW		--		" + clk_info[clk_info_index["name"]] +
                                      "_ea		            --		[0]			--	" + clk_info[clk_info_index["name"]] +
                                      " icg enable")
            if pd.isna(clk_info[clk_info_index["sel"]]) == False:
                print_line.append("--		'h0		--		RW		--		" + clk_info[clk_info_index["name"]] +
                                  "_sel		        --		[8]			--	" + clk_info[clk_info_index["name"]] + " select")
            if pd.isna(clk_info[clk_info_index["div"]]) == False:
                print_line.append("--		'h" + str(hex(int(clk_info[clk_info_index["div_dflt"]]))[2:]) +
                                  "		--		RW		--		" + clk_info[clk_info_index["name"]] + "_divider		    --		[" +
                                  str(int(clk_info[clk_info_index["div_width"]] + 15)) + ":16]		--	" +
                                  clk_info[clk_info_index["name"]] + " divider")
            ...
            if pd.isna(clk_info[clk_info_index["div"]]) == False:
                ...
                print_line.append("\n")
                print_line.append(reg_addr + "    	       --		RW		--		" + clk_info[clk_info_index["name"]] +
                                  "_ea_req				--		[31:0]		--	" + clk_info[clk_info_index["name"]] + " enable request")
                count += 1
                #print(count)
            #if clk_info[clk_info_index["icg"]] == "Y":
            #print_line.append("--		'h0		--		RW		--		"+clk_info[clk_info_index["name"]]+"_icg_ea_req	        --		[0]			--	"+clk_info[clk_info_index["name"]]+" icg enable request")
            if pd.isna(clk_info[clk_info_index["div"]]) == False:
                print_line.append("--		'h0		--		W1T		--		" + clk_info[clk_info_index["name"]] +
                                  "_divider_ea_req	        --		[16]		--	" + clk_info[clk_info_index["name"]] +
                                  " divider enable request")

            if clk_info[clk_info_index["icg"]] == "Y" or pd.isna(clk_info[clk_info_index["sel"]]) == False or pd.isna(
                    clk_info[clk_info_index["div"]]) == False:
                reg_addr = hex(count * 4)
                print_line.append("\n")
                print_line.append(reg_addr + "    	       --		RO		--		" + clk_info[clk_info_index["name"]] +
                                  "_status				--		[31:0]		--	" + clk_info[clk_info_index["name"]] + " status")
                count += 1
            if clk_info[clk_info_index["icg"]] == "Y":
                print_line.append("--		'h0		--		RO		--		" + clk_info[clk_info_index["name"]] +
                                  "_ea_status		    --		[0]	    	--  " + clk_info[clk_info_index["name"]] +
                                  " icg enable status")
                #print_line.append("--		'h0		--		RO		--		"+clk_info[clk_info_index["name"]]+"_icg_ea_ack		    --		[4]	    	--  "+clk_info[clk_info_index["name"]]+" icg enable ack")
            if pd.isna(clk_info[clk_info_index["sel"]]) == False:
                print_line.append("--		'h0		--		RO		--		" + clk_info[clk_info_index["name"]] +
                                  "_sel_clk0_sel	    --		[8]			--	" + clk_info[clk_info_index["name"]] +
                                  " select clk1 status")
                print_line.append("--		'h0		--		RO		--		" + clk_info[clk_info_index["name"]] +
                                  "_sel_clk1_sel	    --		[9]			--	" + clk_info[clk_info_index["name"]] +
                                  " select clk1 status")
                print_line.append("--		'h0		--		RO		--		" + clk_info[clk_info_index["name"]] +
                                  "_sel_done		    --		[10]		--	" + clk_info[clk_info_index["name"]] +
                                  " select done status")
            if pd.isna(clk_info[clk_info_index["div"]]) == False:
                print_line.append("--		'h0		--		RO		--		" + clk_info[clk_info_index["name"]] +
                                  "_divider_done	    --		[12]		--  " + clk_info[clk_info_index["name"]] +
                                  " divider done status")
                #print_line.append("--		'h0		--		RO		--		"+clk_info[clk_info_index["name"]]+"_divider_ea_ack	    --		[13]		--  "+clk_info[clk_info_index["name"]]+" divider enable ack")
                print_line.append("--		'h0		--		RO		--		" + clk_info[clk_info_index["name"]] +
                                  "_divider_status	    --		[" + str(int(clk_info[clk_info_index["div_width"]] + 15)) +
                                  ":16]		--	" + clk_info[clk_info_index["name"]] + " divider status")
    ...
    for idx in rst_reg_num:
        ...
        print_line.append(reg_addr + "				--		RW		--		soft_reset_ctrl" + str(int(count)).rjust(3, '0') +
                          "			    --		[31:0]		--	soft_reset_ctrl" + str(int(count)).rjust(3, '0'))
        #print(count)
        count += 1
        for rst_info in rst_corpus:
            #print(rst_info[rst_info_index["name"]])
            if pd.isna(rst_info[rst_info_index["soft_lc"]]) == True or pd.isna(
                ...
                if rst_info_reg_lc[0] == rst_reg_num[int(idx)]:
                    if rst_info[rst_info_index["soft_dflt"]] == "N":
                        print_line.append("--		'h1		--		RW		--		" + rst_info[rst_info_index["name"]] +
                                          "_sftrstn		                --		[" + rst_info_reg_lc[1] + "]			--	" +
                                          rst_info[rst_info_index["name"]] + "_sftrstn")
                    else:
                        print_line.append("--		'h0		--		RW		--		" + rst_info[rst_info_index["name"]] +
                                          "_sftrstn		                --		[" + rst_info_reg_lc[1] + "]			--	" +
                                          rst_info[rst_info_index["name"]] + "_sftrstn")
        print_line.append("\n")
    ...
    for idx in rst_reg_num:
        ...
        print_line.append(reg_addr + "				--		RO		--		soft_reset_status" + str(int(rst_status_count)).rjust(3, '0') +
                          "				--		[31:0]		--	soft_reset_status" + str(int(rst_status_count)).rjust(3, '0'))
        #print(count)
        count += 1
        rst_status_count += 1
        for rst_info in rst_corpus:
            if pd.isna(rst_info[rst_info_index["soft_lc"]]) == True or pd.isna(
                ...
                rst_info_reg_lc = rst_info[rst_info_index["soft_lc"]].split('-', 1)
                if rst_info_reg_lc[0] == rst_reg_num[int(idx)]:
                    print_line.append("--		'h0		--		RO		--		" + rst_info[rst_info_index["name"]] +
                                      "_status	                    --		[" + rst_info_reg_lc[1] + "]			--	" +
                                      rst_info[rst_info_index["name"]] + "_status")
        print_line.append("\n")
...
def crg_gen_top_csv(rst_info_index, gen_filepath, top_corpus, rst_corpus, rst_ser, clk_info_index, top_info_index,
                    clk_corpus, clk_ser, reg_corpus, reg_ser, code_corpus, code_ser, code_empty, reg_empty, intp_empty,
                    intp_corpus, intp_ser): #{{{
    ...
    #elif top_corpus[top_info_index["protocol"]][1] == "ahb":
    print_line.append("inst " + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol + "_reg u_" +
                      top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol + "_reg")
    print_line.append("#para_inst_begin")
    print_line.append("#para_inst_end")
    print_line.append("#port_inst_begin")
    if protocol == "apb":
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_reg.clk                 ,apb_clk            ,I         ,input,")
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_reg.rst_n               ,apb_rst_n          ,I         ,input,")
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_reg.psel                ,apb_sel            ,I         ,input,")
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_reg.penable             ,apb_enable         ,I         ,input,")
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_reg.pwrite              ,apb_write          ,I         ,input,")
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_reg.paddr               ,apb_addr[31:0]     ,I         ,input,")
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_reg.pwdata              ,apb_wdata[31:0]    ,I         ,input,")
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_reg.prdata              ,apb_rdata[31:0]    ,O         ,output,")
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_reg.pready              ,apb_ready          ,O         ,output,")
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_reg.pslverr             ,apb_slverr          ,O         ,output,")
    elif protocol == "dab":
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_reg.clk                 ,dab_clk            ,I         ,input,")
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_reg.rst_n               ,dab_rst_n          ,I         ,input,")
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_reg.dab_write           ,dab_write          ,I         ,input,")
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_reg.dab_read            ,dab_read           ,I         ,input,")
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_reg.dab_addr            ,dab_addr[31:0]     ,I         ,input,")
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_reg.dab_wdata           ,dab_wdata[31:0]    ,I         ,input,")
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_reg.dab_rdata           ,dab_rdata[31:0]    ,O         ,output,")
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_reg.dab_ready           ,dab_ready          ,O         ,output,")
    elif protocol == "ahb":
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_reg.clk                 ,ahb_clk        ,I         ,input,")
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_reg.rst_n               ,ahb_rst_n      ,I         ,input,")
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_reg.hreadyin            ,hreadyin       ,I         ,input,")
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_reg.hsel                ,hsel           ,I         ,input,")
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_reg.htrans              ,htrans[1:0]    ,I         ,input,")
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_reg.hwrite              ,hwrite         ,I         ,input,")
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_reg.hburst              ,hburst[2:0]    ,I         ,input,")
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_reg.hsize               ,hsize[2:0]     ,I         ,input,")
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_reg.haddr               ,haddr[31:0]    ,I         ,input,")
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_reg.hwdata              ,hwdata[31:0]   ,I         ,input,")
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_reg.hreadyout           ,hreadyout      ,O         ,output,")
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_reg.hresp               ,hresp[1:0]     ,O         ,output,")
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_reg.hrdata              ,hrdata[31:0]   ,O         ,output,")
    ...
    for clk_info in clk_corpus:
        ...
        if clk_info[clk_info_index["attr"]] == "output" or clk_info[clk_info_index["attr"]] == "na" or clk_info[
            ...
            if clk_info[clk_info_index["icg"]] == "Y" or pd.isna(clk_info[clk_info_index["sel"]]) == False or pd.isna(
                ...
                #print(count)
                if clk_info[clk_info_index["icg"]] == "Y":
                    print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" +
                                      protocol + "_regfile." + clk_info[clk_info_index["name"]] + "_ctrl_" +
                                      clk_info[clk_info_index["name"]] + "_ea         ," +
                                      clk_info[clk_info_index["name"]] + "_ea     ,W  ,output,")
#                    print_line.append("connect,"+top_corpus[top_info_index["design_name"]][1].upper()+"_"+protocol+"_regfile."+clk_info[clk_info_index["name"]]+"_ctrl_"+clk_info[clk_info_index["name"]]+"_ea_lock_fld         ,      ,W  ,output,")
                if pd.isna(clk_info[clk_info_index["sel"]]) == False:
                    print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" +
                                      protocol + "_regfile." + clk_info[clk_info_index["name"]] + "_ctrl_" +
                                      clk_info[clk_info_index["name"]] + "_sel        ," +
                                      clk_info[clk_info_index["name"]] + "_sel    ,W  ,output,")
#                    print_line.append("connect,"+top_corpus[top_info_index["design_name"]][1].upper()+"_"+protocol+"_regfile."+clk_info[clk_info_index["name"]]+"_ctrl_"+clk_info[clk_info_index["name"]]+"_sel_lock_fld        ,     ,W  ,output,")
                if pd.isna(clk_info[clk_info_index["div"]]) == False:
                    print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" +
                                      protocol + "_regfile." + clk_info[clk_info_index["name"]] + "_ctrl_" +
                                      clk_info[clk_info_index["name"]] + "_divider_ea_req   ," +
                                      clk_info[clk_info_index["name"]] + "_divider_ea_req     ,W  ,output,")
#                    print_line.append("connect,"+top_corpus[top_info_index["design_name"]][1].upper()+"_"+protocol+"_regfile."+clk_info[clk_info_index["name"]]+"_ctrl_"+clk_info[clk_info_index["name"]]+"_divider_ea_req_lock_fld   ,      ,W  ,output,")
            ...
            if pd.isna(clk_info[clk_info_index["div"]]) == False:
                ...
                print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                                  "_regfile." + clk_info[clk_info_index["name"]] + "_divider_" +
                                  clk_info[clk_info_index["name"]] + "_divider    ," +
                                  clk_info[clk_info_index["name"]] + "_divider[" +
                                  str(int(clk_info[clk_info_index["div_width"]] - 1)) + ":0]    ,W   ,output,")
#                print_line.append("connect,"+top_corpus[top_info_index["design_name"]][1].upper()+"_"+protocol+"_regfile."+clk_info[clk_info_index["name"]]+"_divider_"+clk_info[clk_info_index["name"]]+"_divider_lock_fld    ,     ,W   ,output,")
            if clk_info[clk_info_index["icg"]] == "Y" or pd.isna(clk_info[clk_info_index["sel"]]) == False or pd.isna(
                ...
                if clk_info[clk_info_index["icg"]] == "Y":
                    print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" +
                                      protocol + "_regfile." + clk_info[clk_info_index["name"]] + "_status_" +
                                      clk_info[clk_info_index["name"]] + "_ea_status    ," +
                                      clk_info[clk_info_index["name"]] + "_ea_status  ,W  ,input,")
                ...
                if pd.isna(clk_info[clk_info_index["sel"]]) == False:
                    print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" +
                                      protocol + "_regfile." + clk_info[clk_info_index["name"]] + "_status_" +
                                      clk_info[clk_info_index["name"]] + "_sel_clk0_sel     ," +
                                      clk_info[clk_info_index["name"]] + "_sel_clk0_sel   ,W  ,input,")
                    print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" +
                                      protocol + "_regfile." + clk_info[clk_info_index["name"]] + "_status_" +
                                      clk_info[clk_info_index["name"]] + "_sel_clk1_sel     ," +
                                      clk_info[clk_info_index["name"]] + "_sel_clk1_sel   ,W  ,input,")
                    print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" +
                                      protocol + "_regfile." + clk_info[clk_info_index["name"]] + "_status_" +
                                      clk_info[clk_info_index["name"]] + "_sel_done         ," +
                                      clk_info[clk_info_index["name"]] + "_sel_done       ,W  ,input,")
                if pd.isna(clk_info[clk_info_index["div"]]) == False:
                    print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" +
                                      protocol + "_regfile." + clk_info[clk_info_index["name"]] + "_status_" +
                                      clk_info[clk_info_index["name"]] + "_divider_status   ," +
                                      clk_info[clk_info_index["name"]] + "_divider_status[" +
                                      str(int(clk_info[clk_info_index["div_width"]] - 1)) + ":0]     ,W  ,input,")
                    print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" +
                                      protocol + "_regfile." + clk_info[clk_info_index["name"]] + "_status_" +
                                      clk_info[clk_info_index["name"]] + "_divider_done     ," +
                                      clk_info[clk_info_index["name"]] + "_divider_done   ,W  ,input,")
    ...
    for idx in rst_reg_num:
        ...
        for rst_info in rst_corpus:
            if pd.isna(rst_info[rst_info_index["soft_lc"]]) == True or pd.isna(
                ...
                if rst_info_reg_lc[0] == rst_reg_num[int(count)]:
                    print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" +
                                      protocol + "_regfile." + reg_name[int(count)] + "_" +
                                      rst_info[rst_info_index["name"]] + "_sftrstn     ," +
                                      rst_info[rst_info_index["name"]] + "_sftrstn    ,W  ,output,")
...
#                    print_line.append("connect,"+top_corpus[top_info_index["design_name"]][1].upper()+"_"+protocol+"_regfile."+reg_name[int(count)]+"_"+rst_info[rst_info_index["name"]]+"_lock_fld     ,     ,W  ,output,")
        ...
        count += 1
    for repeat_idx in range(len(print_line) - 1, -1, -1):
        if print_line.count(print_line[repeat_idx]
                            ) > 1 and "connect" in print_line[repeat_idx] and "_lock_fld" in print_line[repeat_idx]:
            print_line.pop(repeat_idx)
    rst_status_count = 0
    count = 0
    for idx in rst_reg_num:
        ...
        for rst_info in rst_corpus:
            if pd.isna(rst_info[rst_info_index["soft_lc"]]) == True or pd.isna(
                ...
                if rst_info_reg_lc[0] == rst_reg_num[int(rst_status_count)]:
                    print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" +
                                      protocol + "_regfile." + reg_name[int(count)] + "_status_" +
                                      rst_info[rst_info_index["name"]] + "_status     ," +
                                      rst_info[rst_info_index["name"]] + "   ,O   ,input,")
        count += 1
        rst_status_count += 1
        ...
        for reg_info in reg_corpus:
            if pd.isna(reg_info[0]) == False:
                ...
                if reg_info[4] == "RO":
                    if int(field_msb) - int(field_lsb) == 0:
                        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" +
                                          protocol + "_regfile." + rst_reg_name + "_" + reg_info[2] + "    ," +
                                          reg_info[2] + "  ,W  ,input,")
                    else:
                        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" +
                                          protocol + "_regfile." + rst_reg_name + "_" + reg_info[2] + "    ," +
                                          reg_info[2] + "[" + str(int(field_msb) - int(field_lsb)) + ":0]  ,W  ,input,")
                elif reg_info[4] == "W1T":
                    print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" +
                                      protocol + "_regfile." + rst_reg_name + "_" + reg_info[2] + "    ," +
                                      reg_info[2] + "  ,W  ,output,")
                    ...
                    if int(field_msb) - int(field_lsb) == 0:
                        ...
                        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" +
                                          protocol + "_regfile." + rst_reg_name + "_" + reg_info[2] + "    ," +
                                          reg_info[2] + "[" + str(int(field_msb) - int(field_lsb)) +
                                          ":0]  ,W  ,output,")
        ...
        for intp_info in intp_corpus:
            if pd.isna(intp_info[0]) == False:
                if intp_count % 7 == 0:
                    ...
                    print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" +
                                      protocol + "_regfile." + str(intp_name_str) + "_out                       ," +
                                      str(intp_name_str) + "_out  ,W  ,output,")
                ...
                field_lsb = bitoffset2[0]
                print_line.append("connect," + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                                  "_regfile." + str(intp_name_str) + "_" + str(intp_info[2]) + "    ," +
                                  str(intp_info[2]) + "  ,W  ,input,")
...
#        print_line.append("connect,"+top_corpus[top_info_index["design_name"]][1].upper()+"_"+protocol+"_regfile."+str(intp_name_str)+"_"+intp_info[2]+"    ,"+intp_info[2]+"["+str(int(field_msb)-int(field_lsb))+":0]  ,W  ,output,")
    ...
    print_line.append("inst " + top_corpus[top_info_index["design_name"]][1] + "_clk_gen u_" +
                      top_corpus[top_info_index["design_name"]][1] + "_clk_gen")
    print_line.append("#para_inst_begin")
    print_line.append("#para_inst_end")
    print_line.append("#port_inst_begin")
    count = 0
    if top_corpus[top_info_index["protocol"]][1] == "apb":
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] +
                          "_clk_gen.apb_clk     ,apb_clk    ,I  ,input,")
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] +
                          "_clk_gen.apb_rst_n   ,apb_rst_n  ,I  ,input,")
    elif top_corpus[top_info_index["protocol"]][1] == "dab":
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] +
                          "_clk_gen.dab_clk     ,dab_clk    ,I  ,input,")
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] +
                          "_clk_gen.dab_rst_n   ,dab_rst_n  ,I  ,input,")
    elif top_corpus[top_info_index["protocol"]][1] == "ahb":
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] +
                          "_clk_gen.ahb_clk     ,ahb_clk    ,I  ,input,")
        print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] +
                          "_clk_gen.ahb_rst_n   ,ahb_rst_n  ,I  ,input,")

    print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] +
                      "_clk_gen.dft_icg_mode_root   ,dft_icg_mode_root  ,I  ,input,")
    for clk_info in clk_corpus:
        count += 1
        ...
        if clk_info[clk_info_index["attr"]] == "input":
            print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] +
                              "_clk_gen.CLK_NAME   ,CLK_NAME   ,I  ,input,")
        ...
        if clk_info[clk_info_index["attr"]] == "internal" and pd.isna(clk_info[clk_info_index["src0"]]) == True:
            print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] +
                              "_clk_gen.CLK_NAME   ,CLK_NAME   ,W  ,input,")
        if clk_info[clk_info_index["attr"]] == "output" or clk_info[clk_info_index["attr"]] == "na" or (
                clk_info[clk_info_index["attr"]] == "internal" and pd.isna(clk_info[clk_info_index["src0"]]) == False):
            if pd.isna(clk_info[clk_info_index["sel"]]) == False:
                #print(pd.isna(clk_info[clk_info_index["sel"]]))
                if "," in clk_info[clk_info_index["sel"]]:
                    clk_sel = clk_info[clk_info_index["sel"]].split(",")
                    print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] + "_clk_gen." +
                                      clk_sel[1] + "            ," + clk_sel[1] + "          ,W  ,input,")
                    print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] +
                                      "_clk_gen.CLK_NAME_sel            ,CLK_NAME_sel          ,W  ,input,")
                else:
                    print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] +
                                      "_clk_gen.CLK_NAME_sel            ,CLK_NAME_sel          ,W  ,input,")
                print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] +
                                  "_clk_gen.CLK_NAME_sel_clk0_sel   ,CLK_NAME_sel_clk0_sel ,W  ,output,")
                print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] +
                                  "_clk_gen.CLK_NAME_sel_clk1_sel   ,CLK_NAME_sel_clk1_sel ,W  ,output,")
                print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] +
                                  "_clk_gen.CLK_NAME_sel_done       ,CLK_NAME_sel_done     ,W  ,output,")
            if pd.isna(clk_info[clk_info_index["div"]]) == False:
                ...
                if pd.isna(clk_info[clk_info_index["div_val_to_en"]]) == False:
                    print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] + "_clk_gen." +
                                      clk_info[clk_info_index["div_val_to_en"]] + "    ," +
                                      clk_info[clk_info_index["div_val_to_en"]] + "   ,W,     input,")
                    other_clk_list = clk_info[clk_info_index["div_val_to"]].split("[")
                    print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] + "_clk_gen." +
                                      other_clk_list[0] + "    ," + other_clk_list[0] + "   ,W,     input,")
                if pd.isna(clk_info[clk_info_index["divider_fadj"]]) == False:
                    print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] + "_clk_gen." +
                                      clk_info[clk_info_index["divider_fadj"]] + "    ," +
                                      clk_info[clk_info_index["divider_fadj"]] + "   ,W,     input,")
                if pd.isna(clk_info[clk_info_index["divider_fadj_val"]]) == False:
                    other_clk_list = clk_info[clk_info_index["divider_fadj_val"]].split("[")
                    print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] + "_clk_gen." +
                                      other_clk_list[0] + "        ," + other_clk_list[0] + "[" +
                                      str(int(int(clk_info[clk_info_index["div_width"]] - 1))) + ":0]   ,W  ,input,")
                print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] +
                                  "_clk_gen.CLK_NAME_divider        ,CLK_NAME_divider[" +
                                  str(int(int(clk_info[clk_info_index["div_width"]] - 1))) + ":0]   ,W  ,input,")
                print_line.append(
                    "connect," + top_corpus[top_info_index["design_name"]][1] +
                    "_clk_gen.CLK_NAME_divider_ea_req ,CLK_NAME_divider_ea_req                                ,W  ,input,"
                )
                print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] +
                                  "_clk_gen.CLK_NAME_divider_status ,CLK_NAME_divider_status[" +
                                  str(int(clk_info[clk_info_index["div_width"]] - 1)) + ":0] ,W  ,output,")
                #print_line.append("\toutput          CLK_NAME_divider_ea_ack,")
                print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] +
                                  "_clk_gen.CLK_NAME_divider_done   ,CLK_NAME_divider_done      ,W  ,output,")
            if clk_info[clk_info_index["icg"]] == "Y":
                if clk_info[clk_info_index["ce_en"]] == "Y":
                    print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] +
                                      "_clk_gen.CLK_NAME_ce             ,CLK_NAME_ce         ,W  ,input,")
                print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] +
                                  "_clk_gen.CLK_NAME_ea             ,CLK_NAME_ea                ,W  ,input,")
                #print_line.append("\tinput           CLK_NAME_icg_ea_req,")
                #print_line.append("\toutput          CLK_NAME_icg_ea_ack,")
                print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] +
                                  "_clk_gen.CLK_NAME_ea_status      ,CLK_NAME_ea_status         ,W  ,output,")
            if pd.isna(clk_info[clk_info_index["icg_external"]]) == False:
                if "[" in clk_info[clk_info_index["icg_external"]]:
                    other_clk_list = clk_info[clk_info_index["icg_external"]].split("[")
                    #print(other_clk_list)
                    print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] + "_clk_gen." +
                                      other_clk_list[0] + "   ," + other_clk_list[0] + "[" + other_clk_list[1] +
                                      "    ,I  ,input,")
                    print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] + "_clk_gen." +
                                      clk_info[clk_info_index["name"]] + "_" + other_clk_list[0] + "   ," +
                                      clk_info[clk_info_index["name"]] + "_" + other_clk_list[0] + "[" +
                                      other_clk_list[1] + "_sync    ,W  ,output,")
                else:
                    print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] + "_clk_gen." +
                                      clk_info[clk_info_index["icg_external"]] + "   ," +
                                      clk_info[clk_info_index["icg_external"]] + "    ,I  ,input,")
                    print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] + "_clk_gen." +
                                      clk_info[clk_info_index["name"]] + "_" +
                                      clk_info[clk_info_index["icg_external"]] + "_sync   ," +
                                      clk_info[clk_info_index["name"]] + "_" +
                                      clk_info[clk_info_index["icg_external"]] + "_sync    ,W  ,output,")
            if pd.isna(clk_info[clk_info_index["icg_internal"]]) == False:
                if "[" in clk_info[clk_info_index["icg_internal"]]:
                    ...
                    print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] + "_clk_gen." +
                                      clk_info[clk_info_index["icg_internal"]] + "   ," +
                                      clk_info[clk_info_index["icg_internal"]] + "    ,W  ,input,")

            if clk_info[clk_info_index["attr"]] == "output":
                print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] +
                                  "_clk_gen.CLK_NAME    ,CLK_NAME   ,O  ,output,")
                #if clk_info[clk_info_index["icg"]] == "Y" :
                if clk_info[clk_info_index["ce_en"]] == "Y":
                    print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] +
                                      "_clk_gen.CLK_NAME_cg_bf          ,CLK_NAME_cg_bf             ,W  ,output,")
            elif clk_info[clk_info_index["attr"]] == "internal":
                print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] +
                                  "_clk_gen.CLK_NAME    ,CLK_NAME   ,W  ,output,")
    ...
    for repeat_idx in range(len(print_line) - 1, -1, -1):
        if print_line.count(print_line[repeat_idx]
                            ) > 1 and "connect" in print_line[repeat_idx] and "_clk_gen" in print_line[repeat_idx]:
            print_line.pop(repeat_idx)
    print_line.append(
        "#inst_begin==========================================================================================================="
    )
    print_line.append("inst " + top_corpus[top_info_index["design_name"]][1] + "_rst_gen u_" +
                      top_corpus[top_info_index["design_name"]][1] + "_rst_gen")
    print_line.append("#para_inst_begin")
    print_line.append("#para_inst_end")
    print_line.append("#port_inst_begin")
    count = 0
    for rst_info in rst_corpus:
        count += 1
        if rst_info[rst_info_index["inout"]] == "input":
            print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] + "_rst_gen." +
                              rst_info[rst_info_index["name"]] + "     ," + rst_info[rst_info_index["name"]] +
                              "    ,I  ,input,")
        ...
        if rst_info[rst_info_index["inout"]] == "output":
            ...
            if rst_info[rst_info_index["soft_src"]] == "SOFT":
                print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] + "_rst_gen." +
                                  rst_info[rst_info_index["name"]] + "_sftrstn     ," +
                                  rst_info[rst_info_index["name"]] + "_sftrstn    ,W  ,input,")
            print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] + "_rst_gen." +
                              rst_info[rst_info_index["name"]] + "     ," + rst_info[rst_info_index["name"]] +
                              "    ,O  ,output,")
        ...
        if pd.isna(rst_info[rst_info_index["external_src"]]) == False:
            if "[" in rst_info[rst_info_index["external_src"]]:
                ...
                #print(other_rst_list)
                print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] + "_rst_gen." +
                                  other_rst_list[0] + "   ," + other_rst_list[0] + "[" + other_rst_list[1] +
                                  "   ,I  ,input,")
            else:
                print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] + "_rst_gen." +
                                  rst_info[rst_info_index["external_src"]] + "     ," +
                                  rst_info[rst_info_index["external_src"]] + "     ,I  ,input,")
        if pd.isna(rst_info[rst_info_index["internal_src"]]) == False:
            if "[" in rst_info[rst_info_index["internal_src"]]:
                ...
                print_line.append("connect," + top_corpus[top_info_index["design_name"]][1] + "_rst_gen." +
                                  rst_info[rst_info_index["internal_src"]] + "     ," +
                                  rst_info[rst_info_index["internal_src"]] + "    ,W  ,input,")
    for repeat_idx in range(len(print_line) - 1, -1, -1):
        if print_line.count(print_line[repeat_idx]
                            ) > 1 and "connect" in print_line[repeat_idx] and "_rst_gen" in print_line[repeat_idx]:
            print_line.pop(repeat_idx)
    print_line.append("#port_inst_end")
    print_line.append(
        "#inst_end============================================================================================================"
    ...
    #        print_line.pop(repeat_idx)
    print_line = _crg_register_csv_contract(
        print_line, top_corpus[top_info_index["design_name"]][1], protocol)
    for line in print_line:
        #print(line)
        fp.write(line)
        fp.write('\n')
...
def crg_gen_top(top_corpus, rst_corpus, rst_ser, clk_info_index, top_info_index, clk_corpus, clk_ser, reg_corpus,
                reg_ser, code_corpus, code_ser, code_empty, reg_empty, intp_empty, intp_corpus, intp_ser): #{{{
    ...
    if top_corpus[top_info_index["protocol"]][1] == "dab":
        print_line.append("\t" + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol + "_regfile u_" +
                          top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_regfile(/*autoinst*/")
        ...
        print_line.append("    " + top_corpus[top_info_index["design_name"]][1].upper() + "_apb_regfile u_" +
                          top_corpus[top_info_index["design_name"]][1].upper() + "_apb_regfile(/*autoinst*/")
        ...
        print_line.append("    " + top_corpus[top_info_index["design_name"]][1].upper() + "_ahb_regfile u_" +
                          top_corpus[top_info_index["design_name"]][1].upper() + "_ahb_regfile(/*autoinst*/")
    ...
    for clk_info in clk_corpus:
        ...
        if clk_info[clk_info_index["attr"]] == "output" or clk_info[clk_info_index["attr"]] == "na" or clk_info[
            ...
            if clk_info[clk_info_index["icg"]] == "Y" or pd.isna(clk_info[clk_info_index["sel"]]) == False or pd.isna(
                ...
                #print(count)
                if clk_info[clk_info_index["icg"]] == "Y":
                    print_line.append("\t\t." + clk_info[clk_info_index["name"]] + "_ctrl_" +
                                      clk_info[clk_info_index["name"]] + "_ea\t\t(" + clk_info[clk_info_index["name"]] +
                                      "_ea),")
...
#                    print_line.append("\t\t."+clk_info[clk_info_index["name"]]+"_ctrl_"+clk_info[clk_info_index["name"]]+"_sel_lock_fld\t\t(          ),")
                if pd.isna(clk_info[clk_info_index["div"]]) == False:
                    print_line.append("\t\t." + clk_info[clk_info_index["name"]] + "_ctrl_" +
                                      clk_info[clk_info_index["name"]] + "_divider\t\t(" +
                                      clk_info[clk_info_index["name"]] + "_divider[" +
                                      str(int(clk_info[clk_info_index["div_width"]] - 1)) + ":0]),")
#                    print_line.append("\t\t."+clk_info[clk_info_index["name"]]+"_ctrl_"+clk_info[clk_info_index["name"]]+"_divider_lock_fld\t\t(          ),")
            if clk_info[clk_info_index["icg"]] == "Y" or pd.isna(clk_info[clk_info_index["div"]]) == False:
                ...
                #print_line.append("\t\t."+clk_info[clk_info_index["name"]]+"_ea_req_"+clk_info[clk_info_index["name"]]+"_icg_ea_req\t\t("+clk_info[clk_info_index["name"]]+"_icg_ea_req),")
                if pd.isna(clk_info[clk_info_index["div"]]) == False:
                    print_line.append("\t\t." + clk_info[clk_info_index["name"]] + "_ea_req_" +
                                      clk_info[clk_info_index["name"]] + "_divider_ea_req\t\t(" +
                                      clk_info[clk_info_index["name"]] + "_divider_ea_req),")
#                    print_line.append("\t\t."+clk_info[clk_info_index["name"]]+"_ea_req_"+clk_info[clk_info_index["name"]]+"_divider_ea_req_lock_fld\t\t(             ),")
            ...
            if clk_info[clk_info_index["icg"]] == "Y" or pd.isna(clk_info[clk_info_index["sel"]]) == False or pd.isna(
                ...
                count += 1
                if clk_info[clk_info_index["icg"]] == "Y":
                    print_line.append("\t\t." + clk_info[clk_info_index["name"]] + "_status_" +
                                      clk_info[clk_info_index["name"]] + "_ea_status\t\t(" +
                                      clk_info[clk_info_index["name"]] + "_ea_status),")
                    #print_line.append("\t\t."+clk_info[clk_info_index["name"]]+"_status_"+clk_info[clk_info_index["name"]]+"_icg_ea_ack\t\t("+clk_info[clk_info_index["name"]]+"_icg_ea_ack),")
                if pd.isna(clk_info[clk_info_index["sel"]]) == False:
    ...
    for idx in rst_reg_num:
        ...
        for rst_info in rst_corpus:
            if pd.isna(rst_info[rst_info_index["soft_lc"]]) == True or pd.isna(
                ...
                if rst_info_reg_lc[0] == rst_reg_num[int(count)]:
                    print_line.append("\t\t." + reg_name[int(count)] + "_" + rst_info[rst_info_index["name"]] +
                                      "_sftrstn\t\t(" + rst_info[rst_info_index["name"]] + "_sftrstn),")
            if rst_info[rst_info_index["soft_src"]] == "SOFT":
                if rst_info_reg_lc[0] == rst_reg_num[int(count)] and "\t\t." + reg_name[int(count)] + "_" + reg_name[
                        int(count)] + "_lock\t\t(           )," not in print_line:
                    print_line.append("\t\t." + reg_name[int(count)] + "_" + reg_name[int(count)] +
                                      "_lock\t\t(           ),")
        count += 1
    reg_addr_ofst = idx
    rst_status_count = 0
    ...
    for idx in rst_reg_num:
        ...
        for rst_info in rst_corpus:
            if pd.isna(rst_info[rst_info_index["soft_lc"]]) == True or pd.isna(
                ...
                rst_info_reg_lc = rst_info[rst_info_index["soft_lc"]].split('-', 1)
                if rst_info_reg_lc[0] == rst_reg_num[int(rst_status_count)]:
                    print_line.append("\t\t." + reg_name[int(count)] + "_status_" + rst_info[rst_info_index["name"]] +
                                      "_status\t\t(" + rst_info[rst_info_index["name"]] + "),")
        count += 1
        rst_status_count += 1
    ...
    if reg_empty == False:
        for reg_info in reg_corpus:
            if pd.isna(reg_info[0]) == False:
                ...
                #    print_line.append("\t\t."+rst_reg_name+"_"+reg_info[2]+"\t\t("+reg_info[2]+"["+str(int(field_msb)-int(field_lsb))+":0]),")
                print_line.append("\t\t." + rst_reg_name + "_" + reg_info[2] + "\t\t(" + reg_info[2] + "[" +
                                  str(int(field_msb) - int(field_lsb)) + ":0]),")
    ...
    print_line.append("\t" + top_corpus[top_info_index["design_name"]][1] + "_clk_gen u_" +
                      top_corpus[top_info_index["design_name"]][1] + "_clk_gen(/*autoinst*/);\n")
    print_line.append("\t" + top_corpus[top_info_index["design_name"]][1] + "_rst_gen u_" +
                      top_corpus[top_info_index["design_name"]][1] + "_rst_gen(/*autoinst*/);\n")
...
def rst_gen_top(para_list, rst_corpus, rst_ser):
    ...
    for rst_info in rst_corpus:
        count += 1
        if rst_info[rst_info_index["attr"]] == "ori_rst" or rst_info[rst_info_index["attr"]] == "test_mode" or rst_info[
                rst_info_index["attr"]] == "test_rstn":
            print_line.append("\tinput           " + rst_info[rst_info_index["name"]] + ",")
    ...
    for idx in rst_reg_num:
        ...
        for rst_info in rst_corpus:
            if pd.isna(rst_info[rst_info_index["soft_lc"]]) == True or pd.isna(
                ...
                rst_info_reg_lc = rst_info[rst_info_index["soft_lc"]].split('-', 1)
                if rst_info_reg_lc[0] == rst_reg_num[int(idx)]:
                    print_line.append("\t\t.soft_reset_ctrl" + str(int(count)).rjust(3, '0') + "_" +
                                      rst_info[rst_info_index["name"]] + "_sftrstn\t\t(" +
                                      rst_info[rst_info_index["name"]] + "_sftrstn),")
        count += 1
    ...
    print_line.append("\t" + top_corpus[top_info_index["design_name"]][1] + " u_" +
                      top_corpus[top_info_index["design_name"]][1] + "(/*autoinst*/);\n")
    print_line.append('endmodule')
    print_line.append('//Local Variables:')
    print_line.append('//verilog-library-directories:(".")')
    print_line.append('//verilog-library-directories:("$XML/")')
...
def clk_gen_top(para_list, clk_corpus, clk_ser):
    ...
    if top_corpus[top_info_index["protocol"]][1] == "dab":
        print_line.append("    " + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_regfile u_" + top_corpus[top_info_index["design_name"]][1].upper() + "_" + protocol +
                          "_regfile(/*autoinst*/")
        print_line.append("    .clk                                   (dab_clk             ), //input")
        print_line.append("    .reset_n                               (dab_rst_n           ), //input")
        print_line.append("    .dab_write                             (dab_write           ), //input")
...
def rst_gen_note(para_list, rst_corpus, rst_ser):
    ...
    print_line.append('// Block')
    if top_corpus[top_info_index["protocol"]][1] == "dab":
        print_line.append(top_corpus[top_info_index["design_name"]][1].upper() + "      --  0x0000:0x0fff  --  " +
                          top_corpus[top_info_index["design_name"]][1] + " regfile       -- dab")
    elif top_corpus[top_info_index["protocol"]][1] == "apb":
        print_line.append(top_corpus[top_info_index["design_name"]][1].upper() + "      --  0x0000:0x0fff  --  " +
                          top_corpus[top_info_index["design_name"]][1] + " regfile       -- apb")
    elif top_corpus[top_info_index["protocol"]][1] == "ahb":
        print_line.append(top_corpus[top_info_index["design_name"]][1].upper() + "      --  0x0000:0x0fff  --  " +
                          top_corpus[top_info_index["design_name"]][1] + " regfile       -- ahb")
    print_line.append('// Register')
    rst_reg_num = []
    rst_reg_bit_lc = []
    for rst_info in rst_corpus:
...
def clk_gen_note(para_list, clk_corpus, clk_ser):
    ...
    for clk_info in clk_corpus:
        if clk_info[clk_info_index["attr"]] == "output" or clk_info[clk_info_index["attr"]] == "na":
            ...
            if clk_info[clk_info_index["icg"]] == "Y":
                print_line.append("--		'h0		--		RW		--		" + clk_info[clk_info_index["name"]] +
                                  "_ea		            --		[0]			--	" + clk_info[clk_info_index["name"]] + " icg enable")
            if pd.isna(clk_info[clk_info_index["sel"]]) == False:
                print_line.append("--		'h0		--		RW		--		" + clk_info[clk_info_index["name"]] +
                                  "_sel		        --		[8]			--	" + clk_info[clk_info_index["name"]] + " select")
            if pd.isna(clk_info[clk_info_index["div"]]) == False:
                print_line.append("--		'h0		--		RW		--		" + clk_info[clk_info_index["name"]] + "_divider		    --		[" +
                                  str(int(clk_info[clk_info_index["div_width"]] + 15)) + ":16]		--	" +
                                  clk_info[clk_info_index["name"]] + " divider")
            ...
            if pd.isna(clk_info[clk_info_index["div"]]) == False:
                print_line.append("--		'h0		--		RO		--		" + clk_info[clk_info_index["name"]] +
                                  ...
                                  ":16]		--	" + clk_info[clk_info_index["name"]] + " divider status")
                print_line.append("--		'h0		--		RO		--		" + clk_info[clk_info_index["name"]] +
                                  "_divider_done	    --		[24]		--  " + clk_info[clk_info_index["name"]] +
                                  " divider done status")
            count += 1
            #print(count)
...
def clk_gen(gen_filepath, top_corpus, clk_info_index, top_info_index, clk_corpus, clk_ser):
        ...
        if clk_info[clk_info_index["attr"]] == "output" or clk_info[clk_info_index["attr"]] == "na" or (
            ...
            if pd.isna(clk_info[clk_info_index["div"]]) == False:
                ...
                if pd.isna(clk_info[clk_info_index["div_val_to_en"]]) == False:
                    ...
                    other_clk_list = clk_info[clk_info_index["div_val_to"]].split("[")
                    print_line.append("\tinput [ " + str(int(int(clk_info[clk_info_index["div_width"]] - 1))) +
                                      ":0]    " + other_clk_list[0] + ",")
                if pd.isna(clk_info[clk_info_index["divider_fadj"]]) == False:
                ...
                print_line.append("\toutput[ " + str(int(clk_info[clk_info_index["div_width"]] - 1)) +
                                  ":0]    CLK_NAME_divider_status,")
                #print_line.append("\toutput          CLK_NAME_divider_ea_ack,")
            ...
            if pd.isna(clk_info[clk_info_index["icg_external"]]) == False:
                if "[" in clk_info[clk_info_index["icg_external"]]:
                    ...
                    #print(other_clk_list)
                    print_line.append("\tinput     [" + other_clk_list[1] + "  " + other_clk_list[0] + ",")
                    print_line.append("\toutput    [" + other_clk_list[1] + "  " + clk_info[clk_info_index["name"]] +
                                      "_" + other_clk_list[0] + "_sync,")
                else:
                    print_line.append("\tinput              " + clk_info[clk_info_index["icg_external"]] + ",")
                    #print_line.append("\toutput             "+clk_info[clk_info_index["icg_external"]]+"_sync,")
                    print_line.append("\toutput             " + clk_info[clk_info_index["name"]] + "_" +
                                      clk_info[clk_info_index["icg_external"]] + "_sync,")
            if pd.isna(clk_info[clk_info_index["icg_internal"]]) == False:
                if "[" in clk_info[clk_info_index["icg_internal"]]:
                    other_clk_list = clk_info[clk_info_index["icg_internal"]].split("[")
            ...
            if clk_info[clk_info_index["attr"]] == "output" or (clk_info[clk_info_index["attr"]] == "internal"
                                                                and pd.isna(clk_info[clk_info_index["src0"]]) == False):
                print_line.append("\toutput          CLK_NAME,")
    ...
    for clk_info in clk_corpus:
        ...
        if clk_info[clk_info_index["attr"]] == "output" or clk_info[clk_info_index["attr"]] == "na" or clk_info[
            ...
            if pd.isna(clk_info[clk_info_index["div"]]) == False:
                ...
                if pd.isna(clk_info[clk_info_index["divider_fadj_val"]]) == False:
                    print_line.append("wire [" + str(int(int(clk_info[clk_info_index["div_width"]] - 1))) +
                                      ":0]    CLK_NAME_divider_fadj_val;")
        ...
        if pd.isna(clk_info[clk_info_index["icg_internal"]]) == False:
            if "[" in clk_info[clk_info_index["icg_internal"]]:
                ...
                print_line.append("wire     " + clk_info[clk_info_index["name"]] + "_" +
                                  clk_info[clk_info_index["icg_internal"]] + "_sync;")
    ...
    for clk_info in clk_corpus:
        ...
        if clk_info[clk_info_index["attr"]] == "output" or clk_info[clk_info_index["attr"]] == "na" or (
            ...
            if pd.isna(clk_info[clk_info_index["sel"]]) == False:
                if clk_info[clk_info_index["mux_dflt"]] == 1:
                    tdr_buf_list.append(design_hier + "u_" + top_corpus[top_info_index["design_name"]][1] +
                                        "_clk_gen/u_" + clk_info[clk_info_index["name"]] +
                                        "_clk_glitch_free_switch/u_dontouch_tdr_buf/u_std_cell_buf 1")
                else:
                    tdr_buf_list.append(design_hier + "u_" + top_corpus[top_info_index["design_name"]][1] +
                                        "_clk_gen/u_" + clk_info[clk_info_index["name"]] +
                                        "_clk_glitch_free_switch/u_dontouch_tdr_buf/u_std_cell_buf 0")
                ...
                print_line.append("")
            if pd.isna(clk_info[clk_info_index["div"]]) == False:
                div_list.append(design_hier + "u_" + top_corpus[top_info_index["design_name"]][1] + "_clk_gen/u_" +
                                clk_info[clk_info_index["name"]] + "_divider_wrap/  #divider_width " +
                                str(int(clk_info[clk_info_index["div_width"]])))
                ...
                for tdr_buf_index in range(int(clk_info[clk_info_index["div_width"]])):
                    ...
                    if (tdr_buf_index >= len(binary_string)):
                        tdr_buf_list.append(design_hier + "u_" + top_corpus[top_info_index["design_name"]][1] +
                                            "_clk_gen/u_" + clk_info[clk_info_index["name"]] +
                                            "_divider_wrap/u_clk_divider_test_tdr_mux/dontouch_tdr_" +
                                            str(tdr_buf_index) + "__u_dontouch_tdr_buf/u_std_cell_buf 0")
                    else:
                        tdr_buf_list.append(design_hier + "u_" + top_corpus[top_info_index["design_name"]][1] +
                                            "_clk_gen/u_" + clk_info[clk_info_index["name"]] +
                                            "_divider_wrap/u_clk_divider_test_tdr_mux/dontouch_tdr_" +
                                            str(tdr_buf_index) + "__u_dontouch_tdr_buf/u_std_cell_buf " +
                                            str(binary_string[int(tdr_buf_index)]))
                ...
                if pd.isna(clk_info[clk_info_index["divider_sync_clk"]]) == False:
                    print_line.append("    .clk_div_sync_clk       (" + clk_info[clk_info_index["divider_sync_clk"]] +
                                      " ), //input")
                    print_line.append("    .clk_in                 (" + clk_info[clk_info_index["src0"]] +
                                      " ), //input")
                ...
                if pd.isna(clk_info[clk_info_index["div_val_to_en"]]) == False:
                    print_line.append("    .clk_div_to_en          (" + clk_info[clk_info_index["div_val_to_en"]] +
                                      " ), //input")
                    print_line.append("    .clk_to_divider         (" + clk_info[clk_info_index["div_val_to"]] +
                                      "    ), //input")
                else:
                    print_line.append("    .clk_div_to_en          (1'b0                  ), //input")
                    print_line.append("    .clk_to_divider         (" +
                                      str(int(clk_info[clk_info_index["div_width"]])) + "'b0 ), //input")
                if pd.isna(clk_info[clk_info_index["divider_fadj"]]) == False:
                    print_line.append("    .clk_divider_ea_req     (CLK_NAME_divider_ea_req  | " +
                                      clk_info[clk_info_index["divider_fadj"]] + "), //input")
                else:
                ...
                print_line.append("    );")
                if pd.isna(clk_info[clk_info_index["divider_fadj_val"]]) == False:
                    print_line.append("assign CLK_NAME_divider_fadj_val = " + clk_info[clk_info_index["divider_fadj"]] +
                                      " ? " + clk_info[clk_info_index["divider_fadj_val"]] + " : CLK_NAME_divider;")
                #if top_corpus[top_info_index["protocol"]][1] == "apb":
            ...
            if clk_info[clk_info_index["occ_scan_mux"]] == "SCAN_MUX":
                ...
                clock_freq = (1 / float(clk_info[clk_info_index["note"]].strip("MHz"))) * 1000
                clock_freq_half = clock_freq / 2
                occ_buf_list.append("clock -name sdr_asic_top." + occ_design_hier + "u_" +
                                    top_corpus[top_info_index["design_name"]][1] + "_clk_gen.u_" +
                                    clk_info[clk_info_index["name"]] + "_buf_for_occ.u_clk_buf_for_occ.Z -domain " +
                                    clk_info[clk_info_index["name"]] + "_domain -atspeed -testclock -period " +
                                    str(int(clock_freq * 10000) / 10000) + " -edge {0 1}")
                ...
                if pd.isna(clk_info[clk_info_index["div"]]) == False:
                    ...
                    print_line.append("    .clkin                  (" + clk_info[clk_info_index["src0"]] +
                                      "     ), //input")
            ...
            if clk_info[clk_info_index["icg"]] == "Y":
                ...
                if pd.isna(clk_info[clk_info_index["icg_external"]]) == False:
                    if "[" in clk_info[clk_info_index["icg_external"]]:
                        ...
                        print_line.append("    for (i = 0; i <= " + width[0] + "; i = i+1)begin:gen_" +
                                          other_clk_list[0] + "_sync")
                        print_line.append("sync ")
                        print_line.append("    #(")
                        print_line.append("    .D_WIDTH                (1                              ),")
                        if top_corpus[top_info_index["delay_beat"]][1] == 3:
                        ...
                        if clk_info[clk_info_index["occ_scan_mux"]] == "SCAN_MUX":
                            ...
                            print_line.append("            .clk_d                  (" +
                                              clk_info[clk_info_index["src0"]] + "     ), //input")
                        print_line.append("            .rst_d_n                (clk_gen_rst_n         ), //input")
                        print_line.append("            .data_s                 (" + other_clk_list[0] +
                                          "[i]          ), //input")
                        print_line.append("            .data_d                 (" + other_clk_list[0] +
                                          "_sync[i]     )  //output")
                        print_line.append("        );")
                        print_line.append("     end")
                        ...
                        print_line.append("    .DATA_DEFAULT           (1'b0                           ) ")
                        print_line.append(")")
                        print_line.append("u_" + clk_info[clk_info_index["name"]] + "_" +
                                          clk_info[clk_info_index["icg_external"]] + "_sync(")
                        if clk_info[clk_info_index["occ_scan_mux"]] == "SCAN_MUX":
                            print_line.append("            .clk_d                  (CLK_NAME_scan_mux   ), //input")
                        elif clk_info[clk_info_index["occ_scan_mux"]] == "OCC":
                            ...
                            print_line.append("            .clk_d                  (" +
                                              clk_info[clk_info_index["src0"]] + "     ), //input")
                        print_line.append("            .rst_d_n                (clk_gen_rst_n         ), //input")
                        print_line.append("            .data_s                 (" +
                                          clk_info[clk_info_index["icg_external"]] + "          ), //input")
                        print_line.append("            .data_d                 (" + clk_info[clk_info_index["name"]] +
                                          "_" + clk_info[clk_info_index["icg_external"]] + "_sync     )  //output")
                        print_line.append("        );")
                if pd.isna(clk_info[clk_info_index["icg_internal"]]) == False:
                    if "[" in clk_info[clk_info_index["icg_internal"]]:
                        other_clk_list = clk_info[clk_info_index["icg_internal"]].split("[")
                        ...
                        print_line.append("            .rst_d_n                (clk_gen_rst_n         ), //input")
                        print_line.append("            .data_s                 (" +
                                          clk_info[clk_info_index["icg_internal"]] + "          ), //input")
                        print_line.append("            .data_d                 (" + clk_info[clk_info_index["name"]] +
                                          "_" + clk_info[clk_info_index["icg_internal"]] + "_sync     )  //output")
                        print_line.append("        );")
                print_line.append("")
                print_line.append("sync ")
                print_line.append("#(")
                    ...
                    if "[" in clk_info[clk_info_index["icg_external"]]:
                        if clk_info[clk_info_index["ce_en"]] == "Y":
                            print_line.append("assign CLK_NAME_ea_multi = CLK_NAME_ea_sync | CLK_NAME_ce & (&" +
                                              other_clk_list[0] + "_sync[" + other_clk_list[1] + ");")
                        else:
                            print_line.append("assign CLK_NAME_ea_multi = CLK_NAME_ea_sync & (&" + other_clk_list[0] +
                                              "_sync[" + other_clk_list[1] + ");")
                    else:
                        if clk_info[clk_info_index["ce_en"]] == "Y":
                            print_line.append("assign CLK_NAME_ea_multi = CLK_NAME_ea_sync | CLK_NAME_ce & " +
                                              clk_info[clk_info_index["name"]] + "_" +
...
def rst_gen(rst_info_index, top_info_index, gen_filepath, top_corpus, rst_corpus, rst_ser):
    ...
    for rst_info in rst_corpus:
        count += 1
        if rst_info[rst_info_index["inout"]] == "output" or rst_info[rst_info_index["inout"]] == "na" or (
                rst_info[rst_info_index["inout"]] == "internal"
                and pd.isna(rst_info[rst_info_index["glb_src"]]) == False):
            print_line.append("//===============")
            ...
            print_line.append("assign " + rst_info[rst_info_index["name"]] + " = " +
                              rst_info[rst_info_index["glb_src"]])
            if rst_info[rst_info_index["soft_src"]] == "SOFT":
...
def replace_CLK_NAME(clk_info, clk_info_index, top_info_index, print_line):
    for element in print_line:
        if 'CLK_NAME' in element:
            print_line[print_line.index(element)] = print_line[print_line.index(element)].replace(
                "CLK_NAME", clk_info[clk_info_index["name"]])
...
def reg_yml_gen(top_info_index, print_line, reg_corpus, reg_ser, top_corpus, reg_addr_ofst):
    ...
    for reg_info in reg_corpus:
        ...
        if pd.isna(reg_info[0]) == False:
            ...
            #print(reg_info)
            if pd.isna(reg_info[7]) == False:
                print_line.append("      - { name: " + reg_info[2] + ", lsb: " + field_lsb + ", bits: " +
                                  str(int(field_msb) - int(field_lsb) + 1) + ", access: " + reg_info[4].lower() +
                                  ", reset: " + reg_info[5] + ", lock_lsb: " + lock_field_lsb + ", lock_bits: " +
                                  str(int(lock_field_msb) - int(lock_field_lsb) + 1) + ", lock_value: " + reg_info[8] +
                                  ", description: \"" + str(reg_info[6]).replace('\n', " ") + "\"}")
            else:
                print_line.append("      - { name: " + reg_info[2] + ", lsb: " + field_lsb + ", bits: " +
                                  str(int(field_msb) - int(field_lsb) + 1) + ", access: " + reg_info[4].lower() +
                                  ", reset: " + reg_info[5] + ", description: \"" +
                                  str(reg_info[6]).replace('\n', " ") + "\"}")
    ...
    for reg_info in reg_corpus:
        ...
        if pd.isna(reg_info[0]) == False:
            ...
            print_line.append(
                ...
                hex(int(reg_info[1], 16) + int(top_corpus[top_info_index["user_defined_reg_addr_ofst"]][1], 16)) +
                "</spirit:addressOffset>")
            print_line.append("      <spirit:size>32</spirit:size>")
            ...
            print_line.append("        <spirit:bitWidth>" + str(int(field_msb) - int(field_lsb) + 1) +
                              "</spirit:bitWidth>")
            print_line.append("        <spirit:access>" + reg_info[4] + "</spirit:access>")
            if pd.isna(reg_info[7]) == False:
                print_line.append("        <spirit:lockOffset>" + lock_field_lsb + "</spirit:lockOffset>")
                print_line.append("        <spirit:lockWidth>" + str(int(lock_field_msb) - int(lock_field_lsb) + 1) +
                                  "</spirit:lockWidth>")
...
def reg_note_gen(print_line, reg_corpus, reg_ser):
    ...
    for reg_info in reg_corpus:
        ...
        if pd.isna(reg_info[0]) == False:
            #print(reg_info)
            print_line.append("")
            print_line.append(reg_info[1] + "    			--		" + reg_info[4] + "		--		" + reg_info[0] +
                              "			    --		[31:0]		--	" + str(reg_info[6]).replace('\n', " "))
        else:
            #print(reg_info[3])
            bitoffset0 = str(reg_info[3]).split('[', 1)
            #print(bitoffset0)
            ...
            if field_msb == field_lsb:
                print_line.append("--		'h" + reg_info[5][2:] + "		--		" + reg_info[4] + "		--		" + reg_info[2] +
                                  "		            --		[" + field_msb + "]			--	" + str(reg_info[6]).replace('\n', " "))
            else:
                print_line.append("--		'h" + reg_info[5][2:] + "		--		" + reg_info[4] + "		--		" + reg_info[2] +
                                  "		            --		[" + field_msb + ":" + field_lsb + "]			--	" +
                                  str(reg_info[6]).replace('\n', " "))
...
def code_gen_csv(print_line, code_corpus, code_ser):
    for code_info in code_corpus:
        ...
        if code_info[0] == "assign":
            ...
            print_line.append("#para_inst_end")
            print_line.append("#port_inst_begin")
            print_line.append("connect," + inst_module + "." + code_info[1] + "     ," + code_info[2] + "   ," +
                              str(code_info[3]) + "  ," + str(code_info[4]) + ",")
    ...
    for intp_info in intp_corpus:
        ...
        if pd.isna(intp_info[0]) == False:
            ...
            if intp_count % 7 == 0:
                ...
                print_line.append(
                    "    offset: " +
                    hex(int(intp_info[1], 16) + int(top_corpus[top_info_index["user_defined_intp_addr_ofst"]][1], 16)))
                print_line.append("    fields:")
            intp_count = intp_count + 1
        elif (intp_count - 1) % 7 == 0:
            #print(intp_count)
            ...
            #print_line.append("      </spirit:field>")
            print_line.append("      - { name: " + intp_info[2] + ", lsb: " + field_lsb + ", bits: " +
                              str(int(field_msb) - int(field_lsb) + 1) + ", access: " + intp_info[4].lower() +
                              ", reset: " + intp_info[5] + ", description: \"" + str(intp_info[6]).replace('\n', " ") +
                              "\"}")
        count = count + 1
