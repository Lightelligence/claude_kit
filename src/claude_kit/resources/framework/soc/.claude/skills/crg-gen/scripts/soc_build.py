...
def deal_with_port_str(line):
    ...
    if '{' in line and '}' in line:
        ...
        logging.debug(
            f"{connect},port_name = {port_name},signal_name = {signal_name},attr = {attr},direction = {direction},checkValue = {checkValue}"
        )
...
class topVerilogGenerator():
    ...
    def write_file(self, fileType="null"):
        ...
        if '/' in self.wfilename and not './' in self.wfilename:
            self.wfilename = os.path.dirname(self.wfilename) + os.sep + os.path.basename(
                self.wfilename).split('.')[0] + '.' + fileType
        else:
...
'''.format(TOPFILE=fileName, MOD=modName, DATE=time.strftime("%Y-%m-%d %H:%M:%S"), USERNAME=authorName, CONTENTS="Init")
    ...
    def gen_port(self, keepPortList):
        ...
        if os.path.dirname(self.rfilename):
            ...
            port_info_bak_csv = os.path.dirname(self.rfilename) + os.sep + "port_info_bak_" + os.path.basename(
                self.rfilename)
        else:
            ...
            for portType in portTypeList:
                ...
                if not subPortType:
                    ...
                    for subType in subPortType:
                        ...
                        tempSubTypeList = [
                            value for key, value in portDict.items() if self.getPortSubType(key, portType, subType)
                        ]
    ...
    def getPortSubType(self, key, portType, subType):
        ...
        for item in temp_port_info:
            if 3 == len(item.strip().split(',')):
                if key == item.strip().split(',')[0] and portType == item.strip().split(
                        ',')[1] and subType == item.strip().split(',')[2]:
                    resFlat = True
                    # self.port_info.remove(key)
        return resFlat
        ...
        if len(line.split(",")) > 2:
            if "input" in line:
                if ",," in line:
                    ...
                    if flag:
                        self.tmp_content += "\t{}\n".format(
                            line.replace(",", " " * 2, 1).replace(",", " " * (30 - segLen), 1))
                    else:
                        self.tmp_content += "\t{},\n".format(
                            line.replace(",", " " * 2, 1).replace(",", " " * (30 - segLen), 1))
            else:
                if ",," in line:
                    ...
                    if flag:
                        self.tmp_content += "\t{}\n".format(
                            line.replace(",", " " * 2, 1).replace(",", " " * (60 - segLen), 1))
                    else:
                        self.tmp_content += "\t{},\n".format(
                            line.replace(",", " " * 2, 1).replace(",", " " * (60 - segLen), 1))
        else:
    ...
    def combo_content(self, module_name, inst_name, inst_para_list, inst_port_list):
        ...
        for index, line in enumerate(inst_port_list):
            ...
            if line.startswith("connect"):
                ...
                if index == len(inst_port_list) - 1:
                    tmp_content += "\t\t{} ({}) //{},{}\n".format(port_name.ljust(60), signal_name.ljust(60),
                                                                  direction.ljust(20), checkValue)
        ...
        tmp_content += "\t);\n"
        self.logging.debug(
            f"wire_list = {self.wire_list} interface_ist = {self.interface_list}, modport_list = {self.modport_list}")
        return (tmp_content)
                        ...
                        if "[" in temp_signal and "]" in temp_signal:
                            ...
                            if signal1.strip() == signal.strip():
                                ...
                                if len(width) == 3:
                                    ...
                                    if value1 > value:
                                        self.logging.debug(
                                            f"pass : value = {value}, value1 = {value1} ,item = {item}\n")
                                        value_flag = True

                                    else:
                                        if temp_counter == 0 and value_flag == False:
                                            self.logging.debug(
                                                f"append : value = {value}, value1 = {value1} ,item = {item}\n")
...
'''
        ...
        for signal_name in list(set(modport_dict.keys())):
            if signal_name in self.interface_list:
                continue
            tmp_content += "\t{} \t\t\t\t{}();\n".format(modport_dict[signal_name].split('.')[0].ljust(60),
                                                         signal_name.strip().replace(' ', ""))
...
class csvGenerator():
    ...
    def find_port_signal(self, content_list):
        ...
        [
            port_content_list.append(line) if "`" in line else port_content_list.append(line.split())
            for line in content_list
        ]
        return port_content_list
    ...
    def pre_process(self, fhandle):
        ...
        [
            self.logging.debug(line.string) if isinstance(line, MicroClass) else self.logging.debug("{} {}".format(
                line.signal_name, line.signal_value)) for line in self.current_parameters
        ]
...
class topCsvGenerator():
        ...
        for line in linelist:
            if "`" in line:
                ...
                if type == "port" or type == "before_port":
                    ...
                    tmp_content += "connect,{}.{},{},{},{},{}\n".format(inst_name, signalname.ljust(30),
                                                                        (connect_name + width).ljust(40),
                                                                        attr.ljust(10), direction, checkValue.rjust(10))
    ...
    def compare_master_list_element(self, srcList, dstList):
        ...
        for item in srcList:
            ...
            self.logging.debug(
                f"flag1 = {flag1},match_value1 = {match_value1}, match_value2 = {match_value2},flag2 = {flag2}")
    ...
    def gen_inst_port_from_top_csv(self, topFile, inst_name, exeCmd, path='./sb_output/'):
        ...
        if "update" in exeCmd:
            ...
            inst_input_list, inst_output_list, inst_mod_list, inst_inoutList, inst_dict_intf = self.get_top_file_one_inst_port(
                topFile, inst_name)
        ...
        ####################################instance master and slave list##########################################################
            ...
            inst_input_list, inst_output_list, inst_mod_list, inst_inoutList, inst_dict_intf = self.get_top_one_inst_port(
                topFile, inst_name)
        ...
        total_input_list, total_output_list, total_mod_list, inoutList, dict_intf = self.get_top_all_inst_port(
            topFile, inst_name)
        ...
        if 'updatec' == exeCmd: # only update as the author modification
            ...
            self.write_hand_csv(topFile, total_input_list, total_output_list, dictWire,
                                list(set(total_inst_match_list)), inst_name, exeCmd)

        else:
            self.write_new_csv(topFile, total_input_list, total_output_list, total_top_wire_list,
                               list(set(total_inst_match_list)), inst_name, exeCmd)

        return

    def write_new_csv(self, new_top_file, total_input_list, total_output_list, total_top_wire_list, intList, inst_name,
                      exeCmd):
                ...
                if writeFlag == True:
                    ...
                    if "input," in line.strip() or "output," in line.strip() or "inout" in line.strip():
                        ...
                        if "input," in line.strip():
                            ...
                            self.logging.debug(
                                f"io_flag = {io_flag},io_flag1 = {io_flag1},direction = {direction},port_width = {port_width}"
                            )
                            ...
                            line_content = "{},{},{},{},{},{}".format(keyword,
                                                                      signal_name.ljust(30), (port_width).ljust(30),
                                                                      wireType.ljust(10), direction, checkValue)
                            totalLine.append(line_content)
                            ...
                            if 'M' == wireType or 'IF' == wireType or 'm' == wireType or 'if' == wireType:
                                ...
                                if io_flag == False:
                                    if 'master' in port_width:
                                        ...
                                        self.logging.debug(
                                            f"signalvalue = {signalvalue},signalvalue1 = {signalvalue1},signalvalue2 = {signalvalue2},wireType = {wireType}"
                                        )
                                        ...
                                        if (wireType_old == 'M' or wireType_old == 'm') and (wireType == 'IF'
                                                                                             or wireType == 'if'):
                                            port_width = port_width.split(".master")[0] + '_m     ' + port_width.split(
                                                ".master")[1]

                                        line_content = "{},{},{},{},{},{}".format(keyword, signal_name.ljust(30),
                                                                                  (port_width).ljust(30),
                                                                                  wireType.ljust(10), direction,
                                                                                  checkValue)
                                        totalLine.append(line_content)
                                        ...
                                        if (wireType_old == 'M' or wireType_old == 'm') and (wireType == 'IF'
                                                                                             or wireType == 'if'):
                                            port_width = port_width.split(".slave")[0] + '_s    ' + port_width.split(
                                                ".slave")[1]
                                        ...
                                        if port_width.strip().endswith('_m'):
                                            ...
                                            if (wireType_old == 'IF' or wireType_old == 'if') and (wireType == 'M'
                                                                                                   or wireType == 'm'):
                                                port_width = port_width.split("_m")[0] + '.master' + port_width.split(
                                                    "_m")[1][5:]

                                            line_content = "{},{},{},{},{},{}".format(
                                                keyword, signal_name.ljust(30), (port_width).ljust(30),
                                                wireType.ljust(10), direction, checkValue)
                                            totalLine.append(line_content)
                                            ...
                                            if (wireType_old == 'IF' or wireType_old == 'if') and (wireType == 'M'
                                                                                                   or wireType == 'm'):
                                                port_width = port_width.split("_s")[0] + '.slave' + port_width.split(
                                                    "_s")[1][4:]
    ...
    def check_port_info_to_csv(self, topFile, totalConflict, totalNoload, totalUndrive, totalFloating,
                               mutli_port_conflict, mutli_wireType_conflict):
                ...
                if line:
                    if validFlag == True and startLable not in line:
                        ...
                        if 'output' in direction:
                            ...
                            self.logging.debug(
                                f"floating_flag = {floating_flag}, connectName = {connectName},checkValue = {checkValue}"
                            )
                        ...
                        line_content = "{},{},{},{},{},{}\n".format(keyword,
                                                                    signal_name.ljust(60), (port_width).ljust(80),
                                                                    wireType.ljust(10), direction,
                                                                    checkValue.strip().rjust(10))
    ...
    def check_multi_port_wireType(self, dictElement):
        ...
        multi_port_list_wire = [
            item for item, count in collections.Counter(collect_name_list_attr).items() if count > 1
        ]
        mutli_wireType_conflict = [
            item for item in multi_port_list_wire if self.get_port_wireType_set(item, dictElement)
        ]
    ...
    def get_port_wireType_set(self, connectName, tempDict):
        ...
        for item in tempDict.keys():
            connectNameValue, width = self.get_width_connection_str(tempDict[item][2].replace(",",
                                                                                              " ").replace(" ", ""))
    ...
    def check_port_info(self, filePath):
        ...
        instInputList, instOutputList, instModList, instIoList, instIfList, wInputList, wOutputList, dictElement = self.get_top_inst_port_attr(
            filePath)
        ...
        self.logging.info(
            f"totalConflict = {totalConflict}\n,totalNoload = {totalNoload}\n,totalUndrive = {totalUndrive}\n,totalFloating = {totalFloating}\n"
        )

        inputList, outputList, inoutList, ifdict = self.check_port_info_to_csv(filePath, totalConflict, totalNoload,
                                                                               totalUndrive, totalFloating,
                                                                               mutli_port_conflict,
                                                                               mutli_wireType_conflict)
        self.logging.debug(
                ...
                if 'connect' in row and len(row.split(',')) != 3:
                    ...
                    if 'input,' in row.strip():
                        ...
                        if "O" in wireType or "o" in wireType:
                            ...
                            self.logging.debug(
                                f"\033[1;32mWarning:output {connectName} is wrong type {wireType} \033[0m")
                    else:
                        if 'master' in row.strip() or 'slave' in row.strip():
                            self.logging.debug(
                                f"signal_name = {signal_name},port_width ------=-------- {port_width},wireType = {wireType}"
                            )
                            ...
                            if 'M' in wireType or 'm' in wireType:
                                ...
                                self.logging.debug(
                                    f"\033[1;32mWarning:mod {port_width} is wrong type {wireType} \033[0m")
                    # else:
    ...
    def get_csv_path(self, topFile, csv_path, exeCmd, fileType="null"):
        ...
        if csvFlag == True:
            ...
            if "update" in exeCmd:
                ...
                print(
                    f"\033[1;32mError: {csv_File} could not be opened. Stop here. Please check it and try again. \033[0m"
                )
                exit()
                ...
                for i in range(len(lines)):
                    if lines[i]:
                        if (i < len(lines) - 1) and start in lines[i] and inst_name in lines[i + 1]:
                            startFlag = True
                            self.logging.info(
                                f"start {i} line = {len(lines)} to {inst_name} check the port,startFlag = {startFlag}")

                        if (i < len(lines) -
                                1) and end in lines[i + 1] and "port_inst_end" in lines[i] and inst_name in lines[i -
                                                                                                                  1]:
                            ...
                            if file_input_list: # add multi port
                                ...
                                for item in file_input_list:
                                    ...
                                    line_content = "connect,{}.{},{},{},{},{}".format(
                                        inst_name, signalname1.ljust(30), (signalname1 + width1).strip().ljust(40),
                                        wireType.ljust(10), direction1, checkValue)
                                    line_content_list.append(line_content)
                        ...
                        if startFlag == True:
                            ...
                            if "input" in lines[i] or "output" in lines[i] or "master" in lines[i] or "slave" in lines[
                                    i] or "inout" in lines[i]:
                                keyword, signal_name, connect_width, wireType, direction, checkValue = deal_with_port_str(
                                    lines[i])
                                ...
                                if inst_name not in signal_name:
                                    ...
                                    print(
                                        f"Error: The Instance {inst_name} of this moudle could not be founded here. Please check if it existed in the {topcsv}."
                                    )
                                ...
                                if file_input_list:
                                    ...
                                    if signalname0.strip() == signalname1: #update for the same signal
                                        ...
                                        self.logging.debug(
                                            f"connectName0 = {connectName0},connectName1 = {connectName1}")
                                        ...
                                        if connectName0.strip() == connectName1:
                                            ...
                                            self.logging.debug(
                                                f"module Value = {tempValue},direction = {direction},direction1 = {direction1},wireType = {wireType}"
                                            )

                                            if direction.strip() == direction1.strip() and flag_eq == True:
                                                line_content_list.append(lines[i])
                                                self.logging.debug(
                                                    f" direction not changed and flag_eq == True ,connectName0 = {connectName0},connectName1 = {connectName1},checkValue = {checkValue}"
                                                )
                                                ...
                                                line_content = "connect,{}.{},{},{},{},{}".format(
                                                    inst_name,
                                                    signalname1.ljust(30), (connectName1 + width1).strip().ljust(40),
                                                    wireType.ljust(10), direction1, checkValue)
                                                line_content_list.append(line_content)
                                                ...
                                                self.logging.debug(
                                                    f" direction changed or width changed ,connectName0 = {connectName0},connectName1 = {connectName1},checkValue = {checkValue}"
                                                )
                                            ...
                                            self.logging.debug(
                                                f"width1 = {width1},direction1 = {direction1},checkValue = {checkValue}"
                                            )

                                        self.logging.debug(
                                            f"signalname0 = {signalname0}!= signalname1 = {signalname1},temp value = {tempValue}"
                                        )
                                        ...
                                        if len(width0) == 5:
                                            curSignalFlag = port_in_list(
                                                (direction.strip() + '%' + signalname0 + width0),
                                                file_input_list) #module csv

                                            curSignalFlag1 = port_in_list(
                                                (direction.strip() + '%' + signalname0 + width0_bak),
                                                file_input_list) #module csv
                                        else:
                                            curSignalFlag = port_in_list(
                                                (direction.strip() + '%' + signalname0 + width0),
                                        ...
                                        if len(width1) == 5:
                                            topSignalFlag = port_in_list(
                                                (direction1.strip() + '%' + signalname1 + width1),
                                                dst_input_list) #top csv

                                            topSignalFlag1 = port_in_list(
                                                (direction1.strip() + '%' + signalname1 + width1_bak),
                                                dst_input_list) #top csv
                                        ...
                                        self.logging.debug(
                                            f"curSignalFlag = {curSignalFlag},topSignalFlag = {topSignalFlag},curSignalFlag1 = {curSignalFlag1},topSignalFlag1 = {topSignalFlag1}"
                                        )
                                        ...
                                        if topSignalFlag == False and topSignalFlag1 == False: #add mod port
                                            ...
                                            line_content = "connect,{}.{},{},{},{},{}".format(
                                                inst_name,
                                                signalname1.ljust(30), (signalname1 + width1).strip().ljust(40),
                                                wireType.ljust(10), direction1, checkValue)
                                            line_content_list.append(line_content)
        ...
        if inputStr[1] == "addcsv":
            ...
            if len(inputStr) == 5:
                ...
                if ".csv" in inst_name or ".sv" in inst_name:
                    print(
                        f"Error: the instance or module name of {inst_name} is wrong, it should be nake name. Please correct it and try again. Thanks."
                    )
                    exit()

                if fileType not in ["v", "sv", "null"]:
                    print(
                        f"Error: please check the file type, it should be v or sv, default is v. Please correct it and try again. Thanks."
                    )
                    exit()
...
def argparse():
    ...
    parser.add_option("-p",
                      "--process",
                      dest="process",
                      help="Output file in process",
                      default='./sb_output/nr_top_out.csv')
    parser.add_option("-o",
                      "--output",
                      dest="output",
                      help="Verilog file of output",
                      default='./sb_output/nr_top_out.v')
    ...
    parser.add_option("-l",
                      "--command",
                      dest="cmd",
                      help="support command line as following format:                 \
