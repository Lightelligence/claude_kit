...
mcp = FastMCP(
    name="crg-gen",
    instructions=("从 Excel 配置生成 CRG（时钟复位生成）RTL 和 SDC 约束。\n"
...
def crg_gen(excel_file: str, output_dir: str = ".") -> str:
    ...
    """
    missing = [name for name in ("pandas", "numpy", "openpyxl") if importlib.util.find_spec(name) is None]
    if missing:
    ...
    target_dir = (Path.cwd().resolve() if output_dir in {".", ""} else Path(output_dir).expanduser().resolve())
