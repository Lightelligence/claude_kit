...
class PllRecommender:
    ...
    @staticmethod
    def find_division(src_freq: float,
                      tgt_freq: float,
        ...
        if self.pad_clocks:
            ...
            for c in self.internal_clocks:
                if c.get("source_type") == "pad":
                    d = c.get("div", 1)
                    div_str = f" /{d}" if d > 1 else ""
                    pad_freq = next((p.get("freq_mhz", "?") for p in self.pad_clocks if p["name"] == c["source"]), "?")
                    report_lines.append(f"  {c['source']} ({pad_freq}MHz){div_str} -> {c['name']}")
