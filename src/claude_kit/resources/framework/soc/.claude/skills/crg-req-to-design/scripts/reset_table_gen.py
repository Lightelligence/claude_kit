...
class ResetTreeGenerator:
    ...
    def _is_source_signal(name: str, note: str) -> bool:
        ...
        if any(kw in name_lower for kw in source_keywords):
            return True
        return ("复位源" in n or "复位控制" in n or "专用复位" in n or n.startswith("外部") or "external" in n)
    ...
    def generate(self) -> List[Dict]:
        ...
        for r in subsystem_resets:
            ...
            if self._is_wdt_reset(note):
                ...
                wdt_source = next((s["name"] for s in source_resets if self._is_wdt_reset(s.get("note", ""))),
                                  "WDT_RST")
                if not src1:
                    src1 = wdt_source
                elif not src2:
