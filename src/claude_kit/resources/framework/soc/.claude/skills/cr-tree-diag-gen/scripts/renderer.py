...
class DrawioRenderer:
    ...
    def render(self, graph: Graph, output_path: str, title: str = "Clock Tree"):
        ...
        # 构建源头索引映射（用于边颜色分配）
        source_names = sorted({name for name, node in graph.nodes.items() if node.node_type.startswith("source")})
    ...
    def _get_page_size(self, graph: Graph) -> tuple:
        if not graph.nodes:
            return (850, 1100)
        max_right = max(node.x + _get_node_width(node.node_type, node.attr) for node in graph.nodes.values())
        max_bottom = max(node.y + _get_node_height(node.node_type, node.attr) for node in graph.nodes.values())
        return (
    ...
    def _add_small_label(self, root, parent_id: str, text: str, x: float, y: float, width: float):
        ...
        cell.set(
            "style",
            "text;html=1;strokeColor=none;fillColor=none;align=center;verticalAlign=middle;whiteSpace=wrap;rounded=0;fontSize=11;fontStyle=1;fontColor=#64748b;"
        )
        cell.set("vertex", "1")
        cell.set("parent", parent_id)
        geo = ET.SubElement(cell, "mxGeometry")
    ...
    def _add_reset_row_labels(self, root, graph: Graph, parent_id: str):
        ...
        for output in sorted(outputs, key=lambda n: n.order):
            ...
            cell.set(
                "style",
                "text;html=1;strokeColor=none;fillColor=none;align=right;verticalAlign=middle;whiteSpace=wrap;rounded=0;fontSize=10;fontStyle=1;fontColor=#94a3b8;"
            )
            cell.set("vertex", "1")
            cell.set("parent", parent_id)
            geo = ET.SubElement(cell, "mxGeometry")
    ...
    def _add_title(self, root, title: str, graph: Graph, parent_id: str):
        ...
        cell.set(
            "style",
            "text;html=1;strokeColor=none;fillColor=none;align=center;verticalAlign=middle;whiteSpace=wrap;rounded=0;fontSize=16;fontStyle=1;fontColor=#2c3e50;"
        )
        cell.set("vertex", "1")
        cell.set("parent", parent_id)
    ...
    def _add_edge(self,
        ...
        style = build_edge_style(stroke_color=stroke_color,
                                 entry_y=entry_y,
                                 entry_x=entry_x,
                                 exit_x=exit_x,
                                 exit_y=exit_y)
