...
class Graph:
    ...
    def _parse_signals(self, rows: List[dict], is_reset: bool) -> dict:
        ...
        for idx, row in enumerate(rows):
            ...
            if skip_key in name:
                continue
            skip_names = [
                "NAME", "SRC0", "SRC1", "MUX_DFLT", "DIV", "DIV_WIDTH", "DIV_DFLT", "OCC", "ICG", "ICG_DFLT", "ATTR",
                "SOFT_DFLT", "RESET GENERATE", "CLOCK GENERATE"
            ]
    ...
    def _remove_isolated_sources(self):
        ...
        unreachable = [
            name for name in self.nodes if name not in reachable and not self.nodes[name].node_type.startswith("source")
        ]
