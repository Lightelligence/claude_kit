...
class HierarchicalLayout:
    ...
    def compute(self, graph: Graph):
        ...
        target_nodes = [n for n in graph.nodes.values() if n.node_type in ("output", "na")]
    ...
    def _compute_reset_layout(self, graph: Graph):
        ...
        root_sources = [n for n in graph.nodes.values() if n.node_type.startswith("source")]
        ...
        for row_idx, output in enumerate(outputs):
            ...
            for prev in prevs:
                ...
                if prev_node.node_type == "soft":
                    ...
                    gate = next(
                        (graph.nodes[src]
                         for src in soft_prevs if src in graph.nodes and graph.nodes[src].node_type == "rst_and"),
                        None,
                    )
    ...
    def _place_reset_local_sources(self, graph: Graph, gate: Node, x: float, row_y: float):
        ...
        local_inputs = [src for src in inputs if src in graph.nodes and graph.nodes[src].node_type == "reset_source"]
    ...
    def _get_max_reset_local_inputs(self, graph: Graph) -> int:
        ...
        for node in graph.nodes.values():
            ...
            local_count = sum(
                ...
                if dst == node.name and src in graph.nodes and graph.nodes[src].node_type == "reset_source")
