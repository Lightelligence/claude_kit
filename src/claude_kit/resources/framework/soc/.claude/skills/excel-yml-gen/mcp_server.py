...
mcp = FastMCP(
    name="excel-yml-gen",
    instructions=("从 Excel 寄存器工作簿生成 YAML 和 Verilog regfile。"),
)
...
def excel_yml_gen(excel_file: str, sheet_name: str, output_dir: str = ".") -> str:
    ...
    if missing:
        raise RuntimeError(f"missing runtime modules {missing}; run scripts/setup_mcp_env.sh")
    excel_path = Path(excel_file).expanduser().resolve()
    if not excel_path.is_file():
        raise ValueError(f"excel_file not found: {excel_path}")
    target_dir = (excel_path.parent /
                  f"{excel_path.stem}_generated" if output_dir == "." else Path(output_dir).expanduser().resolve())
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Excel-to-YAML Register Generator MCP Server")
