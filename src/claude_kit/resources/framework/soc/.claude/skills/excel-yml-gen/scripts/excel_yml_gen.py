...
def main():
    ...
    rtl_gen(para_list, xml_corpus, print_line, reg_corpus, reg_ser, reg_empty, gen_file_name, intp_corpus, intp_ser,
            intp_empty, out_dir)
...
def reg_xml_gen(print_line, reg_corpus, reg_ser):
    ...
    for reg_info in reg_corpus:
        ...
        if pd.isna(reg_info[0]) == False:
            ...
            print_line.append("      </spirit:field>")
            if pd.isna(reg_info[7]) == False and "        <spirit:name>" + reg_info[2] + str(int(count)).rjust(
                    3, '0') + "_lock_fld</spirit:name>" not in print_line:
                print_line.append("      <spirit:field>")
                print_line.append("        <spirit:name>" + reg_info[2] + str(int(count)).rjust(3, '0') +
                                  "_lock_fld</spirit:name>")
                print_line.append("        <spirit:description>" + str(reg_info[6]).replace('\n', " ") +
                                  " lock, PRTC_" + str(int(lock_field_msb)) + "_" + str(int(lock_field_lsb)) + "_" +
                                  str(reg_info[8]) + "</spirit:description>")
                print_line.append("        <spirit:bitOffset>" + lock_field_lsb + "</spirit:bitOffset>")
                print_line.append("        <spirit:bitWidth>" + str(int(lock_field_msb) - int(lock_field_lsb) + 1) +
                                  "</spirit:bitWidth>")
    ...
    if reg_empty == False:
        for reg_info in reg_corpus:
            if pd.isna(reg_info[0]) == False:
                ...
                field_lsb = bitoffset2[0]
                if reg_info[4] == "RO":
                    print_line.append("    input    " + "[" + str(int(field_msb) - int(field_lsb)) + ":0]     " +
                                      reg_info[2] + ",")
                else:
                    print_line.append("    output   " + "[" + str(int(field_msb) - int(field_lsb)) + ":0]     " +
                                      reg_info[2] + ",")
            count = count + 1
        ...
        for reg_info in reg_corpus:
            if pd.isna(reg_info[0]) == False:
                ...
                if "mem_ctrl" in reg_info[2]:
                    ...
                    tdr_buf_list.append("test_tdr_mux #(" + str(int(field_msb) - int(field_lsb) + 1) + ") u_" +
                                        reg_info[2] + "_test_tdr_mux( test_mode, func_" + reg_info[2] + ", " +
                                        reg_info[2] + ");")
                    print_line.append("\t\t." + rst_reg_name + "_" + reg_info[2] + "\t\t(func_" + reg_info[2] + "[" +
                ...
                if "mem_ctrl" in reg_info[2]:
                    ...
                    for i in range(32):
                        if (i >= len(binary_string)):
                            tdr_buf_list.append("u_" + gen_file_name + "/u_" + reg_info[2] +
                                                "_test_tdr_mux/dontouch_tdr_" + str(i) +
                                                "__u_dontouch_tdr_buf/u_std_cell_buf 0")
                        else:
                            tdr_buf_list.append("u_" + gen_file_name + "/u_" + reg_info[2] +
                                                "_test_tdr_mux/dontouch_tdr_" + str(i) +
                                                "__u_dontouch_tdr_buf/u_std_cell_buf " + str(binary_string[i]))
