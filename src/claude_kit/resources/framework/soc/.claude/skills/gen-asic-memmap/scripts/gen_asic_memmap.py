...
def addrblock_yml_gen(para_list, note_corpus, out_dir="./"): #{{{
    ...
    for addrblock_info in note_corpus:
        if pd.isna(
                addrblock_info[1]
        ) == True or addrblock_info[1] == "Slave" or addrblock_info[1] == "RESERVE" or addrblock_info[1] in corder:
...
def gen_sysmaph(para_list, note_corpus, note_ser): #{{{
    ...
    for note_info in note_corpus:
        if pd.isna(
                note_info[1]) == True or note_info[1] == "Slave" or note_info[1] == "RESERVE" or note_info[1] in corder:
            print("Repeat addressblock : " + str(note_info[1]))
            continue
        else:
            print_line.append("#ifndef ASIC_" + note_info[1].upper() + "_BASEADDR")
            print_line.append("  #define ASIC_" + note_info[1].upper() + "_BASEADDR (ASIC_BASEADDR + " +
                              note_info[5].lower() + "UL)")
            print_line.append("#endif")
            print_line.append("")
...
def gen_sysmapsvh(para_list, note_corpus, note_ser): #{{{
    ...
    for note_info in note_corpus:
        if pd.isna(
            ...
            print_line.append("`ifndef ASIC_" + note_info[1].upper() + "_BASEADDR")
            print_line.append("  `define ASIC_" + note_info[1].upper() + "_BASEADDR (ASIC_BASEADDR + 'h" +
                              note_info[5].lower().replace("0x", "") + ")")
            print_line.append("`endif")
            print_line.append("")
            corder.append(note_info[1])
...
def gen_note(para_list, note_corpus, note_ser): #{{{
    ...
    for note_info in note_corpus:
        if "RESERVE" in str(note_info[12]) or "DEFINE NAME" in str(note_info[12]) or "Slave" in note_info[0]:
            ...
            print_line.append(
                str(note_info[12]).upper() + "      --  " + note_info[4].lower() + ":" + note_info[5].lower() +
                "  --   " + note_info[0].lower() + " address block       -- dab")
