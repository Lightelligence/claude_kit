...
mcp = FastMCP(
    ...
    instructions=("从 Excel memmap sheet 生成芯片地址映射 YAML 与 C/SV sysmap header。"),
)
...
def gen_asic_memmap(excel_file: str, project_name: str, output_dir: str = ".") -> str:
    ...
    if missing:
        raise RuntimeError(f"missing runtime modules {missing}; run make mcp-setup / scripts/setup_mcp_env.sh")
