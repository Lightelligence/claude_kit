...
def _emit_reg_class(lines: list, blk: str, reg: dict, width: int, coverage: str) -> None:
    ...
    for f in fields:
        lines.append(f"  const bit[{width - 1}:0] {f['name']}_MASK = "
    ...
    for f in fields:
        lines.append(f"           ref   logic[{width - 1}:0] {f['name']} = {f['name']}_TMP,")
    ...
    for f in fields:
        lines.append(f"    if ({f['name']} !== {width}'hz) "
    ...
    for f in fields:
        lines.append(f"           input logic[{width - 1}:0] {f['name']} = {width}'hz,")
    lines.append("           input uvm_path_e path = UVM_DEFAULT_PATH,")
    lines.append("           input uvm_reg_map map = null,")
    lines.append("           input uvm_sequence_base parent = null,")
    lines.append("           input int prior = -1,")
    ...
    for f in fields:
        lines.append(f"      if ({f['name']} !== {width}'hz) begin "
    ...
    lines.append("      this.read(status, read_data, path, map, parent, prior, extension, fname, lineno);")
    lines.append("      read_data = (read_data & (~write_mask)) | write_data;")
    lines.append("    end else begin")
    lines.append("      read_data = write_data;")
    lines.append("    end")
    lines.append("    this.write(status, read_data, path, map, parent, prior, extension, fname, lineno);")
    ...
    for f in fields:
        ...
        lines.append(f'    this.{f["name"]} = uvm_reg_field::type_id::create("{f["name"]}",, get_full_name());')
        lines.append(f'    this.{f["name"]}.configure(this, {bits}, {lsb}, "{facc}", '
...
def generate_uvm_ral(model: dict) -> str:
    ...
    lines.append(f'  function new(string name = "{blk}", bit is_top_blk = 1, '
    ...
    for reg in regs:
        r = reg["name"]
        lines.append(f'    {r} = reg_{blk}_{r}::type_id::create("{r}",, get_full_name());')
    lines.append("")
    for reg in regs:
    ...
    for reg in regs:
        ...
        for f in reg["fields"]:
            ...
            lines.append(f'    {r}.add_hdl_path_slice("{slice_name}", '
                         f"{int(f['bit_offset'])}, {int(f['bit_width'])});")
    lines.append("")
    lines.append(f'    apb_map = create_map("apb_map", {blk}_BASE, {nbytes}, UVM_LITTLE_ENDIAN);')
    for reg in regs:
    ...
    lines.append("")
    lines.append(f'    ahb_map = create_map("ahb_map", {blk}_BASE, {nbytes}, UVM_LITTLE_ENDIAN);')
    for reg in regs:
        off = _hex_int(reg["offset"])
        facc = _uvm_access(reg.get("access", "RW"))
        rights = "RO" if facc == "RO" else ("WO" if facc == "WO" else "RW")
    ...
    lines.append("")
    lines.append(f'    dab_map = create_map("dab_map", {blk}_BASE, {nbytes}, UVM_LITTLE_ENDIAN);')
    for reg in regs:
        off = _hex_int(reg["offset"])
        facc = _uvm_access(reg.get("access", "RW"))
        rights = "RO" if facc == "RO" else ("WO" if facc == "WO" else "RW")
    ...
    lines.append("      default_map = dab_map;")
    lines.append("    end else begin")
    lines.append('      `uvm_warning("RegModel", '
