...
def _emit_shared_body(lines: list, protocol: str, bus: dict, registers: list, interrupts: list) -> None:
    ...
    for reg in registers:
        ...
        for f in reg["fields"]:
            if _acc(f) in ("RW", "WO", "W1T", "WC"):
                lines.append(f"assign {f['name']}_wr = (sel_addr[31:0] == 32'h{off:x}) & wr_en;")
        if any(_acc(f) in ("RW", "RO") for f in reg["fields"]):
            lines.append(f"assign {reg['name']}_rd = (sel_addr[31:0] == 32'h{off:x}) & rd_en;")
    lines.append("")
    ...
    for intp in interrupts:
        ...
        for suf, delta in INTERRUPT_SUFFIXES:
            ...
            if suf == "RAW":
                ...
                lines.append(f"assign {iname}_{suf}_rd = (sel_addr[31:0] == 32'h{off:x}) & rd_en;")
            elif suf == "STAT":
                ...
                lines.append(f"assign {iname}_{suf}_wr = (sel_addr[31:0] == 32'h{off:x}) & wr_en;")
    ...
    for reg in registers:
        for f in reg["fields"]:
            ...
            if "lockOffset" in f:
                ...
                lines.append(f"        if ({wdata}[{lock_msb}:{lock_lsb}] == {lock_w}'h{lock_val:x})")
    ...
    for reg in registers:
        ...
        for f in reg["fields"]:
            if _acc(f) in ("RO", "RW"):
                lines.append(f"assign {reg['name']}_rdata[{_msb(f)}:{_lsb(f)}] = {f['name']};")
    ...
    for intp in interrupts:
        ...
        for f in fields:
            src = f"{iname}_{f['name']}"
            lines.append(f"assign {src}_SET = {{{_bits(f)}{{{iname}_SET_wr}}}} & {wdata}[{_msb(f)}:{_lsb(f)}];")
            lines.append(f"assign {src}_CLR = {{{_bits(f)}{{{iname}_CLR_wr}}}} & {wdata}[{_msb(f)}:{_lsb(f)}];")
        lines.append("")
        ...
        for f in fields:
            ...
            lines.append("    // sticky: clear by CLR, set by SET or selected event; else hold")
            lines.append(f"    {src}_STAT_NXT = ({src}_STAT & ~{src}_CLR)"
                         f" | {src}_SET"
        ...
        for f in fields:
            src = f"{iname}_{f['name']}"
            lines.append(f"assign {iname}_STAT_rdata[{_msb(f)}:{_lsb(f)}] = {src}_STAT;")
        _fill_unused_rdata(lines, f"{iname}_STAT_rdata", fields)
        for f in fields:
            src = f"{iname}_{f['name']}"
            lines.append(f"assign {iname}_MASK_rdata[{_msb(f)}:{_lsb(f)}] = {src}_MASK;")
        ...
        for f in fields:
            src = f"{iname}_{f['name']}"
            lines.append(f"assign {iname}_MODE_rdata[{_msb(f)}:{_lsb(f)}] = {src}_MODE;")
        _fill_unused_rdata(lines, f"{iname}_MODE_rdata", fields)
        for f in fields:
            src = f"{iname}_{f['name']}"
            lines.append(f"assign {iname}_POLAR_rdata[{_msb(f)}:{_lsb(f)}] = {src}_POLAR;")
