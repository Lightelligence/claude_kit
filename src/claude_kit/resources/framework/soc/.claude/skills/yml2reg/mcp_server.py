...
mcp = FastMCP(
    name="yml2reg",
    instructions=("从 YAML 生成 APB/AHB/DAB regfile RTL、Spirit XML、Excel、C/sysmap 头、"
...
@mcp.tool()
def yml2uvm_ral_top(yaml_file: str, output_dir: str = "", emit_children: bool = True) -> str:
...
def yml2bus_adapters(yaml_file: str = "", output_dir: str = "") -> str:
    ...
    if yaml_file:
        yaml_path = _require_yaml(yaml_file)
        return _python("yml2bus_adapters.py", *_docs_args(yaml_path, output_dir), cwd=str(SCRIPT_DIR))
    args = []
    if output_dir:
        args.extend(["-o", str(Path(output_dir).expanduser().resolve())])
...
def yml2regmap_export(yaml_file: str, output_dir: str = "") -> str:
    """导出 regmap JSON + CSV。"""
    yaml_path = _require_yaml(yaml_file)
    return _python("yml2regmap_export.py", *_docs_args(yaml_path, output_dir), cwd=str(SCRIPT_DIR))
