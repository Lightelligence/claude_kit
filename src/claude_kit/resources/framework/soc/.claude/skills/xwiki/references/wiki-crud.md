...
- Treat `$XWIKI_DIR` as a Markdown knowledge directory, not as a Git worktree to manage.
- Never read, traverse, copy, modify, stage, commit, or otherwise operate on `$XWIKI_DIR/.git`.
- Do not run Git with `$XWIKI_DIR` as its working tree. Publishing the external wiki is a separate, explicit user action.
- All xwiki scans and validation must prune hidden directories before descent; authorized wiki writes target visible Markdown files only.
