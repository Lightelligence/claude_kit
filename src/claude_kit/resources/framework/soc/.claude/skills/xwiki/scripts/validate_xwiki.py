...
def _resolve_wiki_dir(args: argparse.Namespace) -> tuple[Path | None, list[Finding]]:
    ...
    if not path.is_absolute() and args.root:
        path = Path(args.root).expanduser() / path
    ...
    if path.is_symlink() or not path.is_dir():
        return path, [
            Finding(
                "WIKI_DIR_INVALID",
                str(path),
                "wiki directory must exist and must not be a symlink",
            )
        ]
    return path, []
...
def _is_under(path: Path, directory: Path) -> bool:
    try:
        _lexical_path(path).relative_to(_lexical_path(directory))
        return True
    except ValueError:
        return False


def _path_has_symlink(path: Path, directory: Path) -> bool:
    path = _lexical_path(path)
    directory = _lexical_path(directory)
    try:
...
def _contains_hidden_component(target: str) -> bool:
...
def _link_exists(root: Path, source: Path, target: str) -> bool:
    ...
    if not _is_under(candidate, root) or _path_has_symlink(candidate, root):
        return False
    if candidate.is_dir():
        index = candidate / "index.md"
        return not index.is_symlink() and index.exists()
    if candidate.exists():
        return True
    if not candidate.suffix:
        markdown = candidate.with_suffix(".md")
        return not markdown.is_symlink() and markdown.exists()
    return False
...
def _validate_links(root: Path, path: Path, text: str, findings: list[Finding]) -> None:
    for match in LINK_RE.finditer(text):
        ...
        if LOCAL_ABSOLUTE_RE.match(target):
            ...
            continue
        if _contains_hidden_component(target):
            findings.append(Finding("LINK_HIDDEN_PATH", rel_path, f"hidden local link is forbidden: {target}"))
            continue
        if not _link_exists(root, path, target):
...
def _visible_wiki_paths(
    root: Path,
) -> tuple[list[Path], list[Path], set[Path], list[Path]]:
    directories: list[Path] = []
    markdown: list[Path] = []
    directories_with_content: set[Path] = set()
    symlinks: list[Path] = []
    ...
    for current_raw, dirnames, filenames in os.walk(root, topdown=True, followlinks=False):
        ...
        symlinks.extend(
            ...
            if not name.startswith(".") and (current / name).is_symlink()
        )
        symlinks.extend(
            current / name
        ...
        if dirnames or any(name.endswith(".md") for name in visible_files):
            directories_with_content.add(current)
        markdown.extend(current / name for name in visible_files if name.endswith(".md"))

    return directories, markdown, directories_with_content, symlinks
...
def _validate_required_structure(
    root: Path,
    directories: list[Path],
    directories_with_content: set[Path],
    findings: list[Finding],
) -> None:
    index = root / "index.md"
    if not _is_real_file(index):
        findings.append(Finding("INDEX_MISSING", "index.md", "root index.md is required and must not be a symlink"))

    for dirname in REQUIRED_TOP_LEVEL_DIRS:
        directory = root / dirname
        if not _is_real_directory(directory):
            findings.append(Finding("REQUIRED_DIR_MISSING", dirname, f"{dirname}/ directory is required and must not be a symlink"))
            continue
    ...
    if _is_real_directory(de_issue):
        for dirname in REQUIRED_DE_ISSUE_DIRS:
            directory = de_issue / dirname
            rel_dir = f"de_issue/{dirname}"
            if not _is_real_directory(directory):
                findings.append(Finding("REQUIRED_DIR_MISSING", rel_dir, f"{rel_dir}/ directory is required and must not be a symlink"))
                continue
            for filename in RESERVED_NAMES:
                path = directory / filename
                if not _is_real_file(path):
    ...
    for directory in directories:
        if not _directory_requires_local_index(directory, root):
            continue
        if directory not in directories_with_content:
            continue
        for filename in RESERVED_NAMES:
            path = directory / filename
            if not _is_real_file(path):
...
def validate_wiki(root: Path) -> list[Finding]:
    findings: list[Finding] = []
    directories, markdown, directories_with_content, symlinks = _visible_wiki_paths(root)
    findings.extend(
        Finding(
            "SYMLINK_FORBIDDEN",
            _rel(path, root),
            "visible wiki paths must not be symlinks",
        )
        for path in symlinks
    )
    _validate_required_structure(root, directories, directories_with_content, findings)
...
def _print_claude_stop(payload: dict[str, Any]) -> None:
    if payload["ok"]:
        print(json.dumps({"systemMessage": "xwiki wiki validation passed"}, ensure_ascii=False))
        return
            ...
            json.dumps(
                {
                    "hookSpecificOutput": {
                        ...
                        "additionalContext": "xwiki wiki validation passed",
                    },
        ...
        json.dumps(
            {
                ...
                "hookSpecificOutput": {
                    "hookEventName": "PostToolUse",
                    "additionalContext": "Fix the reported xwiki validation errors before continuing.",
                },
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.hook == "claude-file":
        ...
        if wiki_dir is not None and target is not None and _is_under(target, wiki_dir):
            if _path_has_symlink(target, wiki_dir):
                findings.append(
                    Finding(
                        "SYMLINK_FORBIDDEN",
                        _rel(target, wiki_dir),
                        "hook target under the wiki must not traverse a symlink",
                    )
                )
            elif target.suffix != ".md":
                _print_claude_file(_payload(True, wiki_dir, []))
                return 0
        elif target is None or target.suffix != ".md":
            _print_claude_file(_payload(True, wiki_dir, []))
            return 0
        else:
            _print_claude_file(_payload(True, wiki_dir, []))
