---
name: soc-openroad
description: Prepare and run local-default OpenROAD-flow-scripts physical-design handoff for silicon-crew SoC projects. Use when Claude Code needs to generate ORFS config/SDC under pd/openroad, run synth/floorplan/place/cts/route/finish/all stages through the default local ORFS path or explicit container backends, summarize OpenROAD reports/results, or connect a SoC RTL project to OpenROAD.
---
...
This adapter expects the Make-style `de/run/rtl.f` handoff described below.
For a Bazel or differently laid-out project, require an explicitly validated
handoff adapter and real ORFS setup before execution. Do not reorganize the
checkout, invoke the Make build server as a fallback, or treat missing tools
as a reason to create placeholder platform directories.

## Layout contract

Use this scheme for SoC projects:
...
2. Call `soc_openroad_init` with `project_dir`, `module_dir`, `design_name`, platform, clock/reset ports, and period. For a top-level SoC run, provide the actual `module_dir`, `design_name`, and `platform` supplied by the engineer.
