...
mcp = FastMCP(
    name="soc-openroad",
    instructions=("OpenROAD-flow-scripts physical-design handoff for silicon-crew projects. "
...
def _require_active() -> None:
    if os.environ.get(MCP_SERVER_ACTIVE_ENV) != "1":
        raise RuntimeError("OpenROAD-flow-scripts execution must run through the registered "
...
def soc_openroad_init(
    ...
    out_dir = Path(output_dir).expanduser() if output_dir else _default_design_config(
        project, platform, design, "auto").parent
...
def soc_openroad_run(
    ...
    config = Path(design_config).expanduser() if design_config else _default_design_config(
        project, platform, design_name, selected_backend)
    ...
    if selected_backend in {"auto", "docker", "podman"}:
        if engine is None:
            if selected_backend == "auto":
                raise RuntimeError("container backend unavailable: install Docker/Podman, or call "
