...
def parse_single_port(item, inherited=None):
    ...
    if direction_match:
        ...
        raise ValueError("only ANSI-style port declarations are supported; "
    ...
    if re.match(r'^(signed|unsigned)\b', remaining, re.IGNORECASE) or '::' in remaining:
        raise ValueError("signed or package-typed ports require a normalized wrapper: "
                         f"{item!r}")
    ...
    name_match = re.fullmatch(r'([a-zA-Z_][a-zA-Z0-9_$]*)', remaining)
    if name_match:
...
def generate_wrapper(module, wrapper_name=None):
    ...
    if module.params:
        ...
        for i, (pname, pdefault, pwidth) in enumerate(module.params):
            comma = "," if i < len(module.params) - 1 else ""
            declared_width = f" [{pwidth}]" if pwidth else ""
            lines.append(f"    parameter{declared_width} {pname} = {pdefault}{comma}")
        lines.append(") (")
    else:
        lines.append(f"module {wrapper_name} (")
    ...
    if module.params:
        ...
        for i, (pname, pdefault, pwidth) in enumerate(module.params):
            comma = "," if i < len(module.params) - 1 else ""
            lines.append(f"        .{pname:<{max_port_len}} ({pname}){comma}")
        lines.append(f"    ) {instance_name} (")
    else:
        lines.append(f"    {module.name} {instance_name} (")
...
def do_update(config, config_path, top_name_override=None, output_file_override=None, map_file_override=None):
    ...
    if output_file and os.path.exists(output_file):
        ...
        # The old top can still contain instances intentionally removed from
        # config. Keep mapping sync within the current configured module set.
        active_modules = {item['module_name'] for item in config['modules']}
        extracted_map = {key: value for key, value in extracted_map.items()
                         if key.split('.', 1)[0] in active_modules}
        extracted_shared = {
            signal: [key for key in keys if key in extracted_map]
            for signal, keys in extracted_shared.items()
            if sum(key in extracted_map for key in keys) >= 2
        }
    ...
    for module in all_modules:
        ...
        for direction, width, port_name in module.ports:
            ...
            csv_rows.append([mod_name, port_name, direction, width if width else '', signal, shared_with, status])
        ...
        for port_name, (direction, width) in old_ports_dict.items():
            if port_name not in current_port_names:
                csv_rows.append([mod_name, port_name, direction, width if width else '', '', '', 'DEL'])
...
def extract_map_from_top(top_file):
    ...
    for match in re.finditer(inst_pattern, content, re.DOTALL):
        ...
        instances.append({'module': module_name, 'instance': instance_name, 'ports': inst_ports})

    # 只保留真正有共享的信号（连接数 >= 2）
    shared_signals = {k: v for k, v in shared_signals.items() if len(v) >= 2}

    return {'mappings': mappings, 'shared_signals': shared_signals, 'instances': instances}
...
def main():
    ...
    int_parser.add_argument('--no-config', action='store_true', help='不生成 integrate 配置文件')
    ...
    if args.command == 'extract':
        ...
        # 生成JSON输出
        output_data = {'source_file': args.file, 'mappings': valid_mappings}
        ...
        for filepath in args.files:
            ...
            if modules:
                ...
                if matched:
                    ...
                    selected = modules[0]
                all_modules.append(selected)
                module_configs.append({'file': filepath_abs, 'module_name': selected.name})
        ...
        if not args.no_config:
            ...
            port_snapshot = {}
            for module in all_modules:
                port_snapshot[module.name] = [{'direction': d, 'width': w, 'name': n} for d, w, n in module.ports]
            ...
            for module in all_modules:
                for direction, width, port_name in module.ports:
                    ...
                    csv_rows.append(
                        [module.name, port_name, direction, width if width else '', signal, shared_with, ''])
        ...
        for name in common:
            ...
            if curr_dir != snap_dir or curr_w != snap_w:
                modified.append({'name': name, 'old': (snap_dir, snap_w), 'new': (curr_dir, curr_w)})
        ...
        for name in common:
            ...
            if curr_dir != spec_dir or curr_w != spec_w:
                mismatched.append({'name': name, 'expected': (spec_dir, spec_w), 'actual': (curr_dir, curr_w)})
