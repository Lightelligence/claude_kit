...
from mem_generators import ( # noqa: E402
    GeneratedMacro, generate_macro, generator_status,
)
...
def _resolve_orfs_root(catalog: dict) -> Path:
    ...
    if not root.is_dir():
        raise FileNotFoundError(f"ORFS platforms root not found: {root}\n"
...
def match_or_generate_macro(
    ...
    try:
        ...
        print(f"Info: catalog miss {platform} {depth}x{width} "
...
def read_excel_rows(excel_path: Path, sheet_name: str, default_platform: str) -> List[MemReq]:
    xls = pd.ExcelFile(excel_path)
    sheets = [s for s in xls.sheet_names if sheet_name in s]
    if not sheets:
        raise ValueError(f"No sheet matching '{sheet_name}'. Available: {xls.sheet_names}")
    rows: List[MemReq] = []
    for sheet in sheets:
        ...
        if c_type is None or c_depth is None or c_width is None:
            raise ValueError(f"sheet '{sheet}' needs TYPE, NumberOfWords, BitsInWord columns")
        for i, rec in df.iterrows():
            ...
            if depth <= 0 or width <= 0:
                continue
            mtype = _norm_type(tv)
            plat = (str(rec[c_plat]).strip().lower() if c_plat is not None and rec[c_plat] is not None
                    and not (isinstance(rec[c_plat], float) and math.isnan(rec[c_plat])) else default_platform)
            if plat not in SUPPORTED_PLATFORMS:
                raise ValueError(f"row {i}: PLATFORM={plat} not in {SUPPORTED_PLATFORMS}")
            ports = (str(rec[c_ports]).strip().lower() if c_ports is not None and rec[c_ports] is not None
                     and not (isinstance(rec[c_ports], float) and math.isnan(rec[c_ports])) else "")
            bit_write = _truthy(rec[c_bw]) if c_bw is not None else True
            write_size = (int(rec[c_ws]) if c_ws is not None and rec[c_ws] is not None
                          and not (isinstance(rec[c_ws], float) and math.isnan(rec[c_ws])) else
                          (8 if plat == "sky130" else 1))
            name = (str(rec[c_name]).strip() if c_name is not None and rec[c_name] is not None
                    and not (isinstance(rec[c_name], float) and math.isnan(rec[c_name])) else "")
            exact = (str(rec[c_exact]).strip() if c_exact is not None and rec[c_exact] is not None
                     and not (isinstance(rec[c_exact], float) and math.isnan(rec[c_exact])) else "")
...
def emit_beh_tpram(path: Path, mod: str, depth: int, width: int, mask_bits: int, async_clk: bool) -> None:
    aw = addr_width(depth)
    if async_clk:
...
def emit_tpram_wrap_openram(
    ...
    aw = addr_width(depth)
    nmask = max(1, width // max(1, macro.write_size)) if bit_write else max(1, width // max(1, macro.write_size))
    wmask_expr = "wem" if bit_write else f"{{{nmask}{{1'b1}}}}"
    lines = header_lines(path.name)
    if async_clk:
...
def emit_rom_wrap(path: Path, wrap: str, depth: int, width: int, init_hex: str = "") -> None:
    aw = addr_width(depth)
    lines = header_lines(path.name) + [
...
def process_row(
    ...
    if macro is not None:
        ...
        if macro.depth != depth or macro.width != width:
            notes.append(f"logical {depth}x{width} adapted to physical {macro.depth}x{macro.width}")
    else:
        depth, width = req.depth, req.width
    ...
    if mtype == "tpram":
        ...
        if macro is not None and req.platform == "sky130":
            emit_capacity_wrap(path, wrap, macro, depth, width, bit_write, req.platform, dual=True, async_clk=req.async_clk)
            return GenResult(req, wrap, path, macro.name, assets_copied, "; ".join(notes))
    ...
    if mtype == "sfifo":
        ...
        return GenResult(req, wrap, path, macro.name, assets_copied, "; ".join(notes + [f"ram={ram_wrap}"]))
    ...
    if mtype == "asfifo":
        ...
        if not t_path.is_file():
            if macro is not None and req.platform == "sky130":
                emit_capacity_wrap(t_path, t_wrap, macro, depth, width, bit_write, req.platform, dual=True, async_clk=True)
            else:
                beh_mod = f"tpram_{depth}d{width}w_beh"
                beh_path = rtl_dir / f"{beh_mod}.v"
...
def write_report(out: Path, results: List[GenResult]) -> None:
    ...
    # CSV
    lines = ["sheet,row,type,platform,depth,width,wrap,macro,lib,lef,v,notes"]
    jlist = []
    flist = []
    for r in results:
        ...
        v = r.assets.get("v") or ""
        lines.append(f"{r.req.sheet},{r.req.row},{r.req.mem_type},{r.req.platform},"
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    for req in rows:
        ...
        try:
            ...
            results.append(r)
            print(f"OK  [{req.platform}] {req.mem_type} {req.depth}x{req.width} "
