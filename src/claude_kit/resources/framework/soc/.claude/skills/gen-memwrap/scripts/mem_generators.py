...
def resolve_openram() -> Tuple[Optional[Path], str]:
    ...
    return None, ("OpenRAM not found. Set OPENRAM_HOME / OPENRAM_COMPILER / "
...
def _resolve_openram_python() -> str:
    ...
    if env_py and Path(env_py).is_file():
        return env_py
    for cand in (Path(os.environ.get("OPENRAM_HOME", "")).expanduser().resolve().parent / "conda-env" / "bin" /
                 "python", ):
        if cand.is_file():
            return str(cand)
...
def resolve_bsg_fakeram() -> Tuple[Optional[Path], str]:
    ...
    if w:
        return w, f"PATH {w}"
    return None, ("bsg_fakeram not found. Set BSG_FAKERAM to the repo root "
...
def generate_openram(
    ...
    env["PYTHONPATH"] = env["OPENRAM_HOME"] + (os.pathsep + env["PYTHONPATH"] if env.get("PYTHONPATH") else "")
    ...
    if proc.returncode != 0:
        ...
        raise RuntimeError(f"OpenRAM failed (exit {proc.returncode}) for {name}. "
                           f"See {log_path}\n{tail}")
    ...
    if assets.get("v") is None and assets.get("lib") is None:
        raise RuntimeError(f"OpenRAM finished but no .v/.lib found under {gen_dir}. See {log_path}")
...
def generate_bsg_fakeram(
    ...
    if proc.returncode != 0:
        ...
        if proc2.returncode != 0:
            raise RuntimeError(f"bsg_fakeram failed (exit {proc.returncode}/{proc2.returncode}). "
                               f"See {log_path}")
...
def _write_fakeram_lib(path: Path, name: str, depth: int, width: int, aw: int, area_um2: float) -> None:
    ...
    lines = [
        ...
        "    memory() { type : ram; " + f"address_width : {aw}; word_width : {width}; }}",
