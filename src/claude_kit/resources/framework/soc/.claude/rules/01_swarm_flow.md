...
reported by `loop_context.py`, subject to the **DV check menu** in
`.claude/CLAUDE.md`. Dispatch is not permission to run unselected EDA checks;
report `needs-validation` and keep stages open when approval/evidence is absent.
